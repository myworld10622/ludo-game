...
class AnimalRoulette extends REST_Controller
{
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->animal_roullette_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->animal_roullette_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
        ...
        $bet_option_count = $this->AnimalRoulette_model->ViewBetOption($this->data['user_id'], $this->data['game_id']);
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->AnimalRoulette_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->AnimalRoulette_model->AddWallet($this->data['user_id'], $value->amount);
