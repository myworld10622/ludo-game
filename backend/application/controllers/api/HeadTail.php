...
class HeadTail extends REST_Controller
{
    ...
    public function get_active_game_socket_post()
    {
        ...
        if ($game_data) {
            ...
            // if ($increment==1) {
                $data['dragon_bet'] = rand($game_data[0]->random_amount,$game_data[0]->random_amount+4000)+$dragon_bet;
                $data['tiger_bet'] = rand($game_data[0]->random_amount, $game_data[0]->random_amount+4000)+$tiger_bet;
                $data['tie_bet'] = rand($game_data[0]->random_amount,$game_data[0]->random_amount+4000)+$tie_bet;
                $this->HeadTail_model->updateRandomAmount($game_data[0]->id,$data['dragon_bet']);
            // } else {
            //     $data['dragon_bet'] = $total_bet_dragon+$dragon_bet;
            //     $data['tiger_bet'] = $total_bet_tiger+$tiger_bet;
            //     $data['tie_bet'] = $total_bet_tie+$tie_bet;
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->head_tails_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->head_tails_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->HeadTail_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->HeadTail_model->AddWallet($this->data['user_id'], $value->amount);
            
                log_statement($this->data['user_id'], HT, $value->amount, $value->id, 0);
            }
