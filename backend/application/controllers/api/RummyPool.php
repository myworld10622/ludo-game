...
class RummyPool extends REST_Controller
{
    ...
    public function get_table_master_post()
    {
        ...
        $table_data = $this->RummyPool_model->getTableMaster('',$this->data['no_of_players']);
    ...
    {
        if (empty($this->data['user_id']) || empty($this->data['id'])) {
            $data['message'] = 'Invalid Parameter';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        ...
        $isMaster = $this->RummyPool_model->getTableMaster('',$this->data['no_of_players'],$this->data['id']);
        if (empty($isMaster)) {
        ...
        $tables = $this->RummyPool_model->getCustomizeActiveTable($table_amount,$this->data['no_of_players']);
        $seat_position = 1;
    ...
    public function start_game_post()
    {
        ...
        $Cards = $this->RummyPool_model->GetCards((count($new_table_data)*RUMMY_CARDS)+2);
        $joker = $Cards[0]->cards;
        $game_data = [
        ...
        foreach ($new_table_data as $key => $value) {
            ...
            if ($amount!=0) {
                ...
                if(!empty($user[0]->rummy_pool_table_id)){
                    log_statement ($value->user_id, RMY_POOL, -$amount,$user[0]->rummy_pool_table_id,0);
                }
    ...
    public function rejoin_game_post()
    {
        ...
        if(!empty($table_id)){
            log_statement ($this->data['user_id'], RMY_POOL, -$rejoin_amount,$table_id,0);
        }    
        $rejoin_data = [
            'user_id' => $this->data['user_id'],
            'table_id' => $table_id,
    ...
    public function leave_table_post()
    {
        ...
        if ($game) {
            ...
            if (count($game_users)==1) {
                ...
                log_statement($game_users[0]->user_id, RMY_POOL, $user_winning_amt,$user[0]->rummy_pool_table_id,$admin_winning_amt);
            }
        }else{
            $game_users = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
            if (count($game_users)==1) {
                $comission = $this->Setting_model->Setting()->admin_commission;
                ...
                $this->RummyPool_model->MakeWinner(0, 0, $game_users[0]->user_id, $admin_winning_amt);
                $this->RummyPool_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_pool_table_id, $game_users[0]->user_id);
                $this->RummyPool_model->AddToWallet($user_winning_amt, $game_users[0]->user_id);
                log_statement($game_users[0]->user_id, RMY_POOL, $user_winning_amt,$user[0]->rummy_pool_table_id,$admin_winning_amt);
            }
        ...
        if (count($game_users)==1) {
            ...
            if (count($All_table_users)>=2) {
                ...
                if (count($All_table_users)==$exceed_count) {
                    ...
                    log_statement ($winner_user_id, RMY_POOL, $user_winning_amt,$user[0]->rummy_pool_table_id,$admin_winning_amt);
                }
            }
    ...
    public function status_post()
    {
        ...
        $data['discarded_card'] = $this->RummyPool_model->DiscardedGameCard($game_id);
    ...
    public function share_wallet_post()
    {
        ...
        $game = $this->RummyPool_model->getActiveGameOnTable($user->rummy_pool_table_id);
        $GameUser = $this->RummyPool_model->GameUser($game->id);
        if (count($GameUser)>1) {
            foreach ($GameUser as $k => $val) {
        ...
        if ($type==1) {
            ...
            if (!$reject) {
                ...
                foreach ($TableUser as $ke => $val) {
                    ...
                    log_statement ($val->user_id, RMY_POOL, $user_winning_amt,$user[0]->rummy_pool_table_id,$admin_winning_amt);
                    $table_user_data = [
                        'table_id' => $user[0]->rummy_pool_table_id,
                        'user_id' =>$val->user_id
                    ];
    ...
    public function start_game($user_id)
    {
        ...
        $Cards = $this->RummyPool_model->GetCards((count($table_data)*RUMMY_CARDS)+2);
        $game_data = [
            'table_id' => $user[0]->rummy_pool_table_id,
            'amount' => count($table_data)*$amount,
            'joker' => $Cards[0]->cards,
    ...
    public function user_game_history_post()
    {
        ...
        $game = $this->RummyPool_model->getLastGameOnTable($user[0]->rummy_pool_table_id);
        ...
        $data['game_users'] = $this->RummyPool_model->GameOnlyUser($game->id);

        $game_users_cards = array();
        foreach ($data['game_users'] as $key => $value) {
            $declare_log = $this->RummyPool_model->GameLog($game->id, 1, '', $value->user_id);
            $game_users_cards[$key]['user'] = $value;
