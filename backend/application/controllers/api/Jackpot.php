...
class Jackpot extends REST_Controller
{
    ...
    public function get_active_game_socket_post()
    {
        ...
        if ($game_data) {
            ...
            $data['high_card_amount'] = $HighCardAmount;
            $data['pair_amount'] = $PairAmount;
            $data['color_amount'] = $ColorAmount;
            $data['sequence_amount'] = $SequenceAmount;
            $data['pure_sequence_amount'] = $PureSequenceAmount;
            $data['set_amount'] = ($SetAmount) ? $SetAmount : 0;
            ...
            $winners = $this->Jackpot_model->getJackpotWinners(1);
            $random_users = [];  // Initialize $random_users before the loop

            if ($winners) {
                foreach ($winners as $key => $value) {
                    $value->user_data = $this->Jackpot_model->getJackpotBigWinners($value->id);
                    $random_users = $this->Jackpot_model->getJackpotBigWinners($value->id);
                }
            }

            $data['big_winner'] = $random_users;  // Assign $random_users to $data['big_winner'] after the loop
    ...
    public function place_bet_post()
    {
        ...
        if ($bet_id) {
            ...
            log_statement ( $this->data['user_id'], JT,-$this->data['amount'],$bet_id) ;
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->Jackpot_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->Jackpot_model->AddWallet($this->data['user_id'], $value->amount);
            
                log_statement($this->data['user_id'], JT, $value->amount, $value->id, 0);
            }
