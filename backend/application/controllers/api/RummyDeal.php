...
class RummyDeal extends REST_Controller
{
    ...
    public function get_table_post(){
        ...
        $isMaster = $this->RummyDeal_model->getTableMaster('',$this->data['id']);
        if (empty($isMaster)) {
    ...
    public function start_game_post(){
        ...
        $Cards = $this->RummyDeal_model->GetCards((count($new_table_data)*RUMMY_CARDS)+2);
        $game_data = [
        ...
        foreach ($new_table_data as $key => $value) {
            ...
            if ($amount!=0) {
                ...
                if(!empty($user[0]->rummy_deal_table_id)){
                    log_statement ($value->user_id, RMY_DEAL, -$amount,$user[0]->rummy_deal_table_id,0);
                }
    ...
    public function rejoin_game_post(){
        ...
        if(!empty($table_id)){
            log_statement ($this->data['user_id'], RMY_DEAL, -$rejoin_amount,$table_id,0);
        }
    ...
    public function leave_table_post()
    {
        ...
        if ($game) {
            ...
            if (count($game_users)==1) {
                ...
                if(!empty($user[0]->rummy_deal_table_id)){
                    log_statement ($game_users[0]->user_id, RMY_DEAL, $user_winning_amt,$user[0]->rummy_deal_table_id,$admin_winning_amt);
                }
              
            }
        }else{
            $game_users = $this->RummyDeal_model->TableUser($user[0]->rummy_pool_table_id);
            ...
            if (count($game_users)==1) {
                ...
                $this->RummyDeal_model->MakeWinner(0, 0, $game_users[0]->user_id, $admin_winning_amt);
                $this->RummyDeal_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_deal_table_id, $game_users[0]->user_id);
                $this->RummyDeal_model->AddToWallet($user_winning_amt, $game_users[0]->user_id);
                if(!empty($user[0]->rummy_deal_table_id)){
                    log_statement ($game_users[0]->user_id, RMY_DEAL, $user_winning_amt,$user[0]->rummy_deal_table_id,$admin_winning_amt);
    ...
    public function pack_game_post()
    {
        ...
        if($table->winner_id==0){
            ...
            if (count($game_users)==1) {
                ...
                // log_statement ($this->data['user_id'], RMY_DEAL, -$amount,$game->id,0);
                $comission = $this->Setting_model->Setting()->admin_commission;
    ...
    public function declare_back_post()
    {
        ...
        if (count($remain_game_users)<=$declare_count) {
            ...
            // log_message('error', 'Game Count: '.count($table_games).' Game Count: '.$game_count);
            if (count($table_games)>=$game_count) {
                ...
                $this->RummyDeal_model->RemoveTableUser($table_user_data);
                // log_message('error', 'Winner User ID: '.$winner_user_id);
                // log_message('error', 'Loser User ID: '.$loser_user_id);
                ...
                if(!empty($user[0]->rummy_deal_table_id)){
                    log_statement ($winner_user_id, RMY_DEAL, $user_winning_amt,$user[0]->rummy_deal_table_id,$admin_winning_amt);
                }
    ...
    public function status_post()
    {
        ...
        if (!empty($table_id)) {
            ...
            foreach ($points as $key => $value) {
                if ($key%3!=0) {
                    $points_arr[$value->user_id][] = $value;
            ...
            if(!empty($points_arr)){
                for ($z=0; $z < count($points_arr[$table_data[0]->user_id]); $z++) {
                    foreach ($table_data as $ke => $val) {
                        $final_points_arr[] = $points_arr[$val->user_id][$z];
                    }
                }
            }
            
            $data['points'] = $final_points_arr;
        }
    ...
    public function start_game($user_id)
    {
        ...
        $Cards = $this->RummyDeal_model->GetCards((count($table_data)*RUMMY_CARDS)+2);
        $game_data = [
            'table_id' => $user[0]->rummy_deal_table_id,
            'amount' => count($table_data)*$amount,
            'joker' => $Cards[0]->cards,
    ...
    public function user_game_history_post()
    {
        ...
        $game = $this->RummyDeal_model->getLastGameOnTable($user[0]->rummy_deal_table_id);
        ...
        $data['game_users'] = $this->RummyDeal_model->GameOnlyUser($game->id);

        $game_users_cards = array();
        foreach ($data['game_users'] as $key => $value) {
            $declare_log = $this->RummyDeal_model->GameLog($game->id, 1, '', $value->user_id);
            $game_users_cards[$key]['user'] = $value;
