...
class AnderBaharPlus extends REST_Controller
{
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->ander_bahar_plus_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->ander_bahar_plus_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
        ...
        if ($bet_id) {
            ...
            log_statement ( $this->data['user_id'], ABP,-$this->data['amount'],$bet_id) ;
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
