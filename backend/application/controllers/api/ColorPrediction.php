...
class ColorPrediction extends REST_Controller
{
    ...
    public function room_post()
    {
        ...
        if ($room_data) {
            ...
            foreach ($room_data as $key => $value) {
                ...
                $rooms[$key]['online'] = $this->ColorPrediction_model->getRoomOnline($value->id);
            }
    ...
    public function get_active_game_post()
    {
        ...
        $room = $this->ColorPrediction_model->getRoom($this->data['room_id'], $this->data['user_id']);
        if (empty($room)) {
        ...
        $game_data = $this->ColorPrediction_model->getActiveGameOnTable($this->data['room_id']);
        if ($game_data) {
            $game_cards = array();
            if ($game_data[0]->status) {
                $game_cards = $this->ColorPrediction_model->GetGameCards($game_data[0]->id);
            }
            ...
            $my_bet_10 = $this->ColorPrediction_model->TotalBetAmount($game_data[0]->id, GREEN);
            $my_bet_11 = $this->ColorPrediction_model->TotalBetAmount($game_data[0]->id, VIOLET);
            $my_bet_12 = $this->ColorPrediction_model->TotalBetAmount($game_data[0]->id, RED);
            $my_bet_big = $this->ColorPrediction_model->TotalBetAmount($game_data[0]->id, BIG);
            $my_bet_small = $this->ColorPrediction_model->TotalBetAmount($game_data[0]->id, SMALL);
            ...
            $data['last_winning'] = $this->ColorPrediction_model->LastWinningBet($this->data['room_id']);

            // $winners = $this->ColorPrediction_model->getJackpotWinners(1);
            // if ($winners) {
            //     foreach ($winners as $key => $value) {
            //         $value->user_data = $this->ColorPrediction_model->getJackpotBigWinners($value->id);
            //     }
            // }
    ...
    public function leave_room_post()
    {
        ...
        $leave_room = $this->ColorPrediction_model->leave_room($this->data['user_id']);
        if ($leave_room) {
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->color_prediction_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->color_prediction_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
        ...
        $game = $this->ColorPrediction_model->View($this->data['game_id']);
        if (!$game) {
        ...
        $bet_id = $this->ColorPrediction_model->PlaceBet($bet_data);

        if ($bet_id) {
            $this->ColorPrediction_model->MinusWallet($this->data['user_id'], $this->data['amount']);
            log_statement ( $this->data['user_id'], CP,-$this->data['amount'],$bet_id);
    ...
    public function repeat_bet_post()
    {
        ...
        $bet = $this->ColorPrediction_model->ViewBet($this->data['user_id'], $this->data['game_id']);
        if ($bet) {
            $data['message'] = 'One Bet Already Placed';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
        ...
        $last_bet = $this->ColorPrediction_model->ViewBet($this->data['user_id']);
        ...
        // if($bet_id)
        // {
        // $this->ColorPrediction_model->MinusWallet($this->data['user_id'], $last_bet[0]->amount);
        $data['message'] = 'Success';
        // $data['bet_id'] = $bet_id;
    ...
    public function double_bet_post()
    {
        ...
        if ($bet_id) {
            $this->ColorPrediction_model->MinusWallet($this->data['user_id'], $amount);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
            $data['bet'] = $last_bet[0]->bet;
            $data['amount'] = $amount;
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->ColorPrediction_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->ColorPrediction_model->AddWallet($this->data['user_id'], $value->amount);
            
                log_statement($this->data['user_id'], CP, $value->amount, $value->id, 0);
            }
    ...
    //     if ($this->ColorPrediction_model->DeleteBet($bet[0]->id, $this->data['user_id'], $this->data['game_id'])) {
    //         $this->ColorPrediction_model->AddWallet($this->data['user_id'], $bet[0]->amount);
    //         $data['message'] = 'Bet Cancel Successfully';
    //         $user_wallet = $this->Users_model->UserProfile($this->data['user_id']);
    //         $data['wallet'] = $user_wallet[0]->wallet;
    //         $data['code'] = HTTP_OK;
    ...
    public function jackpot_winners_post()
    {
        ...
        $winners = $this->ColorPrediction_model->getJackpotWinners();
        if ($winners) {
            foreach ($winners as $key => $value) {
                $value->user_data = $this->ColorPrediction_model->getJackpotBigWinners($value->id);
            }
    ...
    public function last_winners_post()
    {
        ...
        $winners = $this->ColorPrediction_model->LastWinningBet($this->data['room_id'], 50);
        if ($winners) {
            $data['winners'] = $winners;
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
    ...
    public function myHistory_post()
    {
        ...
        $history = $this->ColorPrediction_model->myHistory($this->data['user_id'], 150);
        if ($history) {
    ...
    public function GameHistory_post()
    {
        ...
        $history = $this->ColorPrediction_model->LastWinningBet(1,50);
        if ($history) {
    ...
    public function get_bet_details_post()
    {
        ...
        $bet_id_details = $this->ColorPrediction_model->get_bet_details($bet_id,$user_id);
