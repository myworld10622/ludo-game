...
class DragonTiger extends REST_Controller
{
    ...
    public function room_post()
    {
        ...
        if ($room_data) {
            ...
            foreach ($room_data as $key => $value) {
                ...
                $rooms[$key]['online'] = $this->DragonTiger_model->getRoomOnline($value->id);
            }
    ...
    public function get_active_game_post()
    {
        ...
        $room = $this->DragonTiger_model->getRoom($this->data['room_id'], $this->data['user_id']);
        if (empty($room)) {
        ...
        $game_data = $this->DragonTiger_model->getActiveGameOnTable($this->data['room_id']);
        if ($game_data) {
            $game_cards = array();
            if ($game_data[0]->status) {
                $game_cards = $this->DragonTiger_model->GetGameCards($game_data[0]->id);
            }
    ...
    public function get_active_game_socket_post()
    {
        ...
        if ($game_data) {
            ...
            $data['online'] = rand(300, 350)+count($data['online_users']);
            $data['last_bet'] = $this->DragonTiger_model->ViewBet('', $game_data[0]->id, '', '', 1);
            $data['my_dragon_bet'] = $this->DragonTiger_model->TotalBetAmount($game_data[0]->id, 0, '');
            $data['my_tiger_bet'] = $this->DragonTiger_model->TotalBetAmount($game_data[0]->id, 1, '');
            $data['my_tie_bet'] = $this->DragonTiger_model->TotalBetAmount($game_data[0]->id, 2, '');
            ...
            // if ($increment==1) {
                ...
                $this->DragonTiger_model->updateRandomAmount($game_data[0]->id,$data['dragon_bet']);
            // } else {
            //     $data['dragon_bet'] = $total_bet_dragon+$dragon_bet;
            //     $data['tiger_bet'] = $total_bet_tiger+$tiger_bet;
            //     $data['tie_bet'] = $total_bet_tie+$tie_bet;
    ...
    public function leave_room_post()
    {
        ...
        $leave_room = $this->DragonTiger_model->leave_room($this->data['user_id']);
        if ($leave_room) {
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->dragon_tiger_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->dragon_tiger_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
        ...
        $game = $this->DragonTiger_model->View($this->data['game_id']);
        if (!$game) {
        ...
        // $bet = $this->DragonTiger_model->ViewBet($this->data['user_id'], $this->data['game_id'], $this->data['bet']);
        ...
        $bet_id = $this->DragonTiger_model->PlaceBet($bet_data);

        if ($bet_id) {
            $this->DragonTiger_model->MinusWallet($this->data['user_id'], $this->data['amount']);
            log_statement ( $this->data['user_id'], DT,-$this->data['amount'],$bet_id) ;
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
    ...
    public function repeat_bet_post()
    {
        ...
        $bet = $this->DragonTiger_model->ViewBet($this->data['user_id'], $this->data['game_id']);
        if ($bet) {
            $data['message'] = 'One Bet Already Placed';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
        ...
        $last_bet = $this->DragonTiger_model->ViewBet($this->data['user_id']);
        ...
        // if($bet_id)
        // {
        // $this->DragonTiger_model->MinusWallet($this->data['user_id'], $last_bet[0]->amount);
        $data['message'] = 'Success';
        // $data['bet_id'] = $bet_id;
    ...
    public function double_bet_post()
    {
        ...
        if ($bet_id) {
            $this->DragonTiger_model->MinusWallet($this->data['user_id'], $amount);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
            $data['bet'] = $last_bet[0]->bet;
            $data['amount'] = $amount;
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->DragonTiger_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->DragonTiger_model->AddWallet($this->data['user_id'], $value->amount);
            
                log_statement($this->data['user_id'], DT, $value->amount, $value->id, 0);
            }
    ...
    //     if ($this->DragonTiger_model->DeleteBet($bet[0]->id, $this->data['user_id'], $this->data['game_id'])) {
    //         $this->DragonTiger_model->AddWallet($this->data['user_id'], $bet[0]->amount);
    //         $data['message'] = 'Bet Cancel Successfully';
    //         $user_wallet = $this->Users_model->UserProfile($this->data['user_id']);
    //         $data['wallet'] = $user_wallet[0]->wallet;
    //         $data['code'] = HTTP_OK;
