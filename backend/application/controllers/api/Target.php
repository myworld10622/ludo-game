...
class Target extends REST_Controller
{
    ...
    public function room_post()
    {
        ...
        if ($room_data) {
            ...
            foreach ($room_data as $key => $value) {
                ...
                $rooms[$key]['online'] = $this->Target_model->getRoomOnline($value->id);
            }
    ...
    public function get_active_game_post()
    {
        ...
        $room = $this->Target_model->getRoom($this->data['room_id'], $this->data['user_id']);
        if (empty($room)) {
        ...
        $game_data = $this->Target_model->getActiveGameOnTable($this->data['room_id']);
        if ($game_data) {
            $game_cards = array();
            if ($game_data[0]->status) {
                $game_cards = $this->Target_model->GetGameCards($game_data[0]->id);
            }
            ...
            $data['online'] = $this->Target_model->getRoomOnline($this->data['room_id']);
            $data['online_users'] = $this->Target_model->getRoomOnlineUser($this->data['room_id']);

            $my_bet_0 = $this->Target_model->TotalBetAmount($game_data[0]->id, 0);
            $my_bet_1 = $this->Target_model->TotalBetAmount($game_data[0]->id, 1);
            $my_bet_2 = $this->Target_model->TotalBetAmount($game_data[0]->id, 2);
            $my_bet_3 = $this->Target_model->TotalBetAmount($game_data[0]->id, 3);
            $my_bet_4 = $this->Target_model->TotalBetAmount($game_data[0]->id, 4);
            $my_bet_5 = $this->Target_model->TotalBetAmount($game_data[0]->id, 5);
            $my_bet_6 = $this->Target_model->TotalBetAmount($game_data[0]->id, 6);
            $my_bet_7 = $this->Target_model->TotalBetAmount($game_data[0]->id, 7);
            $my_bet_8 = $this->Target_model->TotalBetAmount($game_data[0]->id, 8);
            $my_bet_9 = $this->Target_model->TotalBetAmount($game_data[0]->id, 9);

            $data['last_bet'] = $this->Target_model->ViewBet('', $game_data[0]->id, '', '', 1);
            ...
            $data['last_winning'] = $this->Target_model->LastWinningBet($this->data['room_id']);

            // $winners = $this->Target_model->getJackpotWinners(1);
            // if ($winners) {
            //     foreach ($winners as $key => $value) {
            //         $value->user_data = $this->Target_model->getJackpotBigWinners($value->id);
            //     }
            // }
    ...
    public function leave_room_post()
    {
        ...
        $leave_room = $this->Target_model->leave_room($this->data['user_id']);
        if ($leave_room) {
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
    ...
    public function place_bet_post()
    {
        ...
        $game = $this->Target_model->View($this->data['game_id']);
        if (!$game) {
        ...
        $bet = $this->Target_model->ViewBet($this->data['user_id'], $this->data['game_id'], $this->data['bet']);
        ...
        $bet_id = $this->Target_model->PlaceBet($bet_data);

        if ($bet_id) {
            $this->Target_model->MinusWallet($this->data['user_id'], $this->data['amount']);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
    ...
    public function repeat_bet_post()
    {
        ...
        $bet = $this->Target_model->ViewBet($this->data['user_id'], $this->data['game_id']);
        if ($bet) {
            $data['message'] = 'One Bet Already Placed';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
        ...
        $last_bet = $this->Target_model->ViewBet($this->data['user_id']);
        ...
        // if($bet_id)
        // {
        // $this->Target_model->MinusWallet($this->data['user_id'], $last_bet[0]->amount);
        $data['message'] = 'Success';
        // $data['bet_id'] = $bet_id;
    ...
    public function double_bet_post()
    {
        ...
        if ($bet_id) {
            $this->Target_model->MinusWallet($this->data['user_id'], $amount);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
            $data['bet'] = $last_bet[0]->bet;
            $data['amount'] = $amount;
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->Target_model->DeleteBet($bet[0]->id, $this->data['user_id'], $this->data['game_id'])) {
            $this->Target_model->AddWallet($this->data['user_id'], $bet[0]->amount);
    ...
    public function jackpot_winners_post()
    {
        ...
        $winners = $this->Target_model->getJackpotWinners();
        if ($winners) {
            foreach ($winners as $key => $value) {
                $value->user_data = $this->Target_model->getJackpotBigWinners($value->id);
            }
    ...
    public function last_winners_post()
    {
        ...
        $winners = $this->Target_model->LastWinningBet($this->data['room_id'], 50);
        if ($winners) {
            $data['winners'] = $winners;
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
