...
class Game extends REST_Controller
{
    ...
    public function join_table_post()
    {
        ...
        if ($user[0]->table_id) {
            ...
            $data['message'] = 'You are Already On Table';
            $data['boot_value'] = $table->boot_value;
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
    ...
    public function start_game_post()
    {
        ...
        foreach ($table_data as $key => $value) {
            ...
            log_statement ($value->user_id, TP, -$amount,
            $GameId,0);
    ...
    public function chaal_post()
    {
        ...
        if ($chaal==$this->data['user_id']) {
            ...
            if ($table->pot_limit <= ($game->amount+$amount)) {
                ...
                $this->Game_model->Chaal($game->id, $amount, $this->data['user_id'], $plus);
                if (count($game_users)==2) {
                    $bot_id = $this->Game_model->getGameBot($game->id);
                    if ($bot_id) {
                        sleep(rand(6,10));
                        $botChaalCount = $this->Game_model->ChaalCount($game->id, $bot_id);
                        if($botChaalCount>rand(6,15)){
                            ...
                            if ($winner==2) {
                                if ($remain_game_users[0]->user_id==$bot_id) {
                            ...
                            $this->Game_model->Show($game->id, $amount, $bot_id);
                            $comission = $this->Setting_model->Setting()->admin_commission;
                            $this->Game_model->MakeWinner($game->id, $game->amount+$amount, $user_id, $comission);
                            $data['bot'] = 1;
                        }else{
    ...
    public function status_post()
    {
        ...
        if ($game) {
            ...
            $slide_show_accepted = $this->Game_model->GetSlideShow($game->id, 1);

            if(!empty($slide_show_accepted)){
                $data['slide_show_from_cards'] = $this->Game_model->GameAllUser($game_id, $slide_show_accepted[0]['user_id']);
                $data['slide_show_to_cards'] = $this->Game_model->GameAllUser($game_id, $slide_show_accepted[0]['prev_id']);
            }
        }
    ...
    public function create_post()
    {
        ...
        $this->db->insert('response_log', $res_data);

        $data['GameId'] = "ANDRO".rand(1000,2000);
        
        $CustomRoomProperties['GameType'] = "CUSTOM_GAME_TYPE";
        $CustomRoomProperties['CustomData'] = "1001";

        $RoomOptions['CustomRoomProperties'] = $CustomRoomProperties;

        $data['EnterRoomParams']['RoomOptions'] = $RoomOptions;
