...
class RummyCacheta extends REST_Controller
{
    private $data;
    ...
    public function get_table_post()
    {
        ...
        if ($user[0]->rummy_cacheta_table_id) {
            $table_data = $this->RummyCacheta_model->TableUser($user[0]->rummy_cacheta_table_id);
        ...
        $isMaster = $this->RummyCacheta_model->getTableMaster($this->data['boot_value'],$this->data['no_of_players']);
        if (empty($isMaster)) {
        ...
        $tables = $this->RummyCacheta_model->getCustomizeActiveTable($table_amount,$this->data['no_of_players']);
        $seat_position = 1;

        if ($tables) {
            foreach ($tables as $value) {
                if ($value->members<$isMaster[0]->no_of_players) {
                    $TableId = $value->rummy_cacheta_table_id;
                    $seat_position = $this->RummyCacheta_model->GetSeatOnTable($TableId);
                    if(!$seat_position){
                        $seat_position = 1;
                        $TableId = '';
                        break;
        ...
        if (empty($TableId)) {
            ...
            $TableId = $this->RummyCacheta_model->CreateTable($table_data);
            ...
            if (!empty($admin_mobile)) {
                ...
                if ($admin_user) {
                    if (!empty($admin_user->fcm)) {
                        ...
                        $fcm_data['title'] = "New User On Rummy Cacheta Table Boot Value ".$table_amount;
            ...
            if ($robot_rummy==0) {
                ...
                if ($bot) {
                    ...
                    $this->RummyCacheta_model->AddTableUser($table_bot_data);
                }
            }
        }
        ...
        $this->RummyCacheta_model->AddTableUser($table_user_data);

        $table_data = $this->RummyCacheta_model->TableUser($TableId);
    ...
    public function get_private_table_post()
    {
        ...
        $table_data_user = $this->RummyCacheta_model->TableUser($TableId);
    ...
    public function get_table_master_post()
    {
        ...
        $table_data = $this->RummyCacheta_model->getTableMaster('',$this->data['no_of_players']);
    ...
    public function join_table_post()
    {
        ...
        if ($user[0]->rummy_cacheta_table_id) {
            $data['message'] = 'You are Already On Table';
            $data['table_data'][0]['table_id'] = $user[0]->rummy_cacheta_table_id;
            $data['table_id'] = $user[0]->rummy_cacheta_table_id;
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }

        if (!$this->RummyCacheta_model->isTable($this->data['table_id'])) {
        ...
        $table = $this->RummyCacheta_model->isTableAvail($this->data['table_id']);
        if (!$table) {
            $data['message'] = 'Invalid Table Id';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
        ...
        $table_user_data = [
            ...
            'user_id' => $user[0]->id,
            'seat_position' => $this->RummyCacheta_model->GetSeatOnTable($this->data['table_id']),
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];
        ...
        $table_data = $this->RummyCacheta_model->TableUser($this->data['table_id']);
    ...
    public function join_table_with_code_post()
    {
        ...
        // if (!$this->RummyCacheta_model->isTable($this->data['code'])) {
        //     $data['message'] = 'Invalid Table Id';
        //     $data['code'] = HTTP_NOT_ACCEPTABLE;
        //     $this->response($data, 200);
        //     exit();
        ...
        $table = $this->RummyCacheta_model->isTableAvailCode($this->data['code']);
        if (!$table) {
            $data['message'] = 'Invalid Table Id';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
        ...
        $table_user_data = [
            'table_id' => $table->id,
            'user_id' => $user[0]->id,
            'seat_position' => $this->RummyCacheta_model->GetSeatOnTable($table->id),
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];
        ...
        $table_data = $this->RummyCacheta_model->TableUser($table->id);
    ...
    public function start_game_post()
    {
        ...
        $game = $this->RummyCacheta_model->getActiveGameOnTable($user[0]->rummy_cacheta_table_id);
        ...
        $table = $this->RummyCacheta_model->isTableAvail($user[0]->rummy_cacheta_table_id);
        ...
        if (count($table_data)>2) {
            foreach ($table_data as $key => $value) {
                if ($value->wallet<$boot_value) {
                    $table_user_data = [
                        'table_id' => $value->rummy_cacheta_table_id,
        ...
        $amount = 0;
        $Cards = $this->RummyCacheta_model->GetStartCards((count($table_data)*RUMMY_CACHETA_CARDS)+2);
        $game_data = [
            'table_id' => $user[0]->rummy_cacheta_table_id,
            'joker' => $Cards[0]->cards,
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];

        $GameId = $this->RummyCacheta_model->Create($game_data);
        ...
        $this->RummyCacheta_model->StartDropGameCards($table_user_data);

        $end = 2;
        foreach ($table_data as $key => $value) {
            $start = $end;
            $end = $end+RUMMY_CACHETA_CARDS;
            for ($i=$start; $i < $end; $i++) {
                $table_user_data = [
                    'game_id' => $GameId,
                    'user_id' => $value->user_id,
            ...
            $this->RummyCacheta_model->AddGameCount($value->user_id);
            ...
            $this->RummyCacheta_model->AddGameLog($game_log);
        }
    ...
    public function leave_table_post()
    {
        ...
        if ($game) {
            $isUserGame = $this->RummyCacheta_model->GameOnlyUser($game->id, $user[0]->id);
            if ($isUserGame) {
                $table = $this->RummyCacheta_model->isTableAvail($user[0]->rummy_cacheta_table_id);
                $boot_value = $table->boot_value;
                $ChaalCount = $this->RummyCacheta_model->ChaalCount($game->id, $this->data['user_id']);

                $percent = $ChaalCount>0 ? CHAAL_PERCENT : NO_CHAAL_PERCENT;
                $amount = round(($percent / 100) * $boot_value, 2);

                $this->RummyCacheta_model->PackGame($this->data['user_id'], $game->id, $timeout, '', $amount, $percent);
                $this->RummyCacheta_model->MinusWallet($this->data['user_id'], $amount);
        ...
        $table_users = $this->RummyCacheta_model->TableUser($user[0]->rummy_cacheta_table_id);
    ...
    public function pack_game_post()
    {
        ...
        $this->RummyCacheta_model->PackGame($this->data['user_id'], $game->id, $timeout, $this->input->post('json'), $amount, $percent);
        $this->RummyCacheta_model->MinusWallet($this->data['user_id'], $amount);
    ...
    public function get_card_post()
    {
        ...
        $cards = $this->RummyCacheta_model->getMyCards($game->id, $user_id);
    ...
    public function drop_card_post()
    {
        ...
        $isCardAvailable = $this->RummyCacheta_model->getMyCards($game->id, $user_id, $card);
        ...
        if ($isCardAvailable) {
            ...
            $this->RummyCacheta_model->DropGameCards($table_user_data, $this->input->post('json'));
            $game_users = $this->RummyCacheta_model->GameAllUser($game->id);
            if (count($game_users)==2) {
                $bot_id = $this->RummyCacheta_model->getGameBot($game->id);
                if ($bot_id) {
                    $data['bot'] = 1;
                }
            }
    ...
    public function get_drop_card_post()
    {
        ...
        $drop_card = $this->RummyCacheta_model->GetAndDeleteGameDropCard($game->id);
    ...
    public function declare_post()
    {
        ...
        if (!$user->rummy_cacheta_table_id) {
            $data['message'] = 'You Are Not On Table';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        ...
        $game = $this->RummyCacheta_model->getActiveGameOnTable($user->rummy_cacheta_table_id);
        if (!$game) {
            $data['message'] = 'Game Not Started';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
    ...
    //     $remain_game_users = $this->RummyCacheta_model->GameUser($game->id);
    ...
    //         $comission = $this->Setting_model->Setting()->admin_commission;
    //         // $amount = $game->amount;
    //         $this->RummyCacheta_model->MinusWallet($user_id,$points);
    //         $this->RummyCacheta_model->MakeWinner($game->id,$points,$user_id,$comission);
    ...
    public function declare_back_post()
    {
        ...
        if (count($remain_game_users)<=$declare_count) {
            ...
            $this->RummyCacheta_model->MinusWallet($this->data['user_id'], $actual_points);
            $comission = $this->Setting_model->Setting()->admin_commission;
    ...
    public function chat_post()
    {
        ...
        if (!empty($chat)) {
            ...
            $this->RummyCacheta_model->Chat($chat_data);
        }

        $chat_list = $this->RummyCacheta_model->ChatList($game->id);
        $data['message'] = 'Success';
        $data['list'] = $chat_list;
        $data['code'] = HTTP_OK;
        $this->response($data, HTTP_OK);
    ...
    public function tip_post()
    {
        ...
        if ($this->Users_model->TipAdmin($this->data['tip'], $this->data['user_id'], $user[0]->rummy_cacheta_table_id)) {
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
            exit();
    ...
    public function card_value_post()
    {
        ...
        $active_game = $this->RummyCacheta_model->getActiveGameOnTable($user[0]->rummy_cacheta_table_id);
        $joker = "";
        if ($active_game) {
        ...
        $card_value = $this->RummyCacheta_model->CardValue('', $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
        if ($card_value) {
            // echo $joker;
            // print_r($card_value);
            if ($card_value[0]==0) {
                $card_value = $this->RummyCacheta_model->CardValue($joker, $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
            }
    ...
    public function status_post()
    {
        ...
        if (!empty($table_id)) {
            $table_data = $this->RummyCacheta_model->TableUser($table_id);
            ...
            $active_game = $this->RummyCacheta_model->getActiveGameOnTable($table_id);
            if ($active_game) {
                // $game_id = $active_game->id;
                $data['active_game_id'] = $active_game->id;
                $data['game_status'] = 1;
        ...
        $game = $this->RummyCacheta_model->View($game_id);
        if (empty($game)) {
        ...
        $game_log = $this->RummyCacheta_model->GameLog($game_id, 1);

        $game_users = $this->RummyCacheta_model->GameAllUser($game_id);
        ...
        if ($game_log[0]->action==3) {
            ...
            // $data['game_users'] = $this->RummyCacheta_model->GameAllUser($game_id);
        }
        // else
        // {
        $data['game_users'] = $this->RummyCacheta_model->GameOnlyUser($game_id);
        // }
        $data['chaal'] = $chaal;
        $data['game_amount'] = $game->amount;
        $data['last_card'] = $this->RummyCacheta_model->LastGameCard($game->id);
        $chaalCount = $this->RummyCacheta_model->ChaalCount($game->id, $chaal);
        $percent = $chaalCount>0 ? CHAAL_PERCENT : NO_CHAAL_PERCENT;
        $data['cut_point'] = round(($percent / 100) * MAX_POINTS, 2);
        // if ($chaalCount>3) {
        //     $this->RummyCacheta_model->getMyCards($game->id, $chaal);
        // }

        if (!empty($user_id)) {
            // $data['cards'] = $this->RummyCacheta_model->getMyCards($game->id,$user_id);
            $data['drop_card'] = $this->RummyCacheta_model->GetGameDropCard($game->id);
        ...
        if ($game->winner_id>0) {
            ...
            foreach ($data['game_users'] as $key => $value) {
                $declare_log = $this->RummyCacheta_model->GameLog($game->id, 1, '', $value->user_id);
                ...
                $game_users_cards[$key]['user']->cards = json_decode($this->RummyCacheta_model->GameLogJson($game->id, $value->user_id));
            }
    ...
    public function user_card_post()
    {
        ...
        foreach ($game_users as $key => $value) {
            $cards = $this->RummyCacheta_model->getMyCards($game->id, $value->user_id);
    ...
    public function swap_card_post()
    {
        ...
        $isCardAvailable = $this->RummyCacheta_model->getMyCards($game->id, $user_id, $my_card);

        if ($isCardAvailable) {
            $this->RummyCacheta_model->SwapCards($user_id, $game->id, $my_card, $new_card);
    ...
    public function get_available_cards_post()
    {
        ...
        $data['message'] = 'Success';
        $data['code'] = HTTP_OK;
        $data['cards_list'] = $this->RummyCacheta_model->GetGameCard($game->id);
        $data['joker'] = $game->joker;
        $this->response($data, HTTP_OK);
        exit();
