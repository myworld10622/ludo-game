...
class AnderBahar extends REST_Controller
{
    ...
    public function get_active_game_socket_post()
    {
        ...
        if ($game_data) {
            ...
            // if ($increment==1) {
                $data['ander_bet'] = rand($game_data[0]->random_amount, $game_data[0]->random_amount+4000)+$ander_bet;
                $data['bahar_bet'] = rand($game_data[0]->random_amount, $game_data[0]->random_amount+4000)+$bahar_bet;
                $this->AnderBahar_model->updateRandomAmount($game_data[0]->id,$data['ander_bet']);
            // } else {
            //     $data['ander_bet'] = $total_bet_ander+$ander_bet;
            //     $data['bahar_bet'] = $total_bet_ander+$bahar_bet;
            // }
    ...
    public function place_bet_post()
    {
        ...
        if ($user[0]->wallet < $setting->ander_bahar_min_bet) {
            $data['message'] = 'Required Minimum ' . $setting->ander_bahar_min_bet . ' Coins to Play';
            $data['code'] = HTTP_NOT_ACCEPTABLE;
            $this->response($data, 200);
            exit();
        }
        ...
        if ($bet_id) {
            ...
            log_statement ( $this->data['user_id'], AB,-$this->data['amount'],$bet_id);
            $data['message'] = 'Success';
            $data['bet_id'] = $bet_id;
    ...
    public function cancel_bet_post()
    {
        ...
        if ($this->AnderBahar_model->DeleteBet($this->data['user_id'], $this->data['game_id'])) {

            foreach ($bet as $key => $value) {
                $this->AnderBahar_model->AddWallet($this->data['user_id'], $value->amount);
            
                log_statement($this->data['user_id'], AB, $value->amount, $value->id, 0);
            }
