                ...
                <div class="card-body table-responsive">
                    <table id="datatable" class="table table-bordered dt-responsive table-bordered nowrap"
                        ...
                        <tbody>
                            <?php
                            $i = 0;
                            foreach ($DistributorManualGatway as $key => $distributormanualgatway) {
                                $i++;
                                ?>
                                <tr>
                                    <td><?= $i ?></td>
                                    <td><?= $distributormanualgatway->name ?></td>
                                    <td><?= $distributormanualgatway->number ?></td>
                                    <?php if ($_ENV['ENVIRONMENT'] != 'demo') { ?>
                                        <td>
                                            <a href="<?= base_url('backend/Gateway/editDistributorManual/' . $distributormanualgatway->id) ?>"
