...
class LudoOld_model extends MY_Model
{
    ...
    public function GameLog($game_id, $step = '')
    {
        $this->db->from('tbl_ludo_log');
        $this->db->where('game_id', $game_id);
        $this->db->order_by('id', 'DESC');
    ...
    public function MakeWinner($game_id, $win_amount, $user_id, $comission)
    {
        ...
        log_statement ($user_id, LD, $user_winning_amt,
    ...
    public function getActiveGameOnTableDuringKickOut($Invite_code)
    {
        $this->db->select('tbl_ludo.*');
        $this->db->from('tbl_ludo');
        ...
        $this->db->where('tbl_ludo_table.invite_code', $Invite_code);
