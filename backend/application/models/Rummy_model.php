...
class Rummy_model extends MY_Model
{
    ...
    public function getTableMaster($boot_value='',$no_of_players='')
    {
        ...
        $this->db->join('tbl_rummy_table', 'tbl_rummy_table_master.boot_value=tbl_rummy_table.boot_value AND tbl_rummy_table.isDeleted=0 AND `tbl_rummy_table`.`no_of_players` = '.$no_of_players, 'left');
    ...
    {
        $this->db->select('tbl_users.rummy_table_id,COUNT(tbl_rummy_table.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_table', 'tbl_users.rummy_table_id=tbl_rummy_table.id');
        $this->db->where('tbl_users.isDeleted', false);
    ...
    {
        $sql = "SELECT * FROM ( SELECT 1 AS mycolumn UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 ) a WHERE mycolumn NOT in ( SELECT seat_position FROM `tbl_rummy_table_user` WHERE table_id=" . $TableId . " AND isDeleted=0 ) LIMIT 1";
    ...
    {
        $this->db->select('tbl_rummy_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type,tbl_rummy_table.no_of_players,tbl_rummy_table.boot_value');
        $this->db->from('tbl_rummy_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_table_user.user_id=tbl_users.id');
        $this->db->join('tbl_rummy_table', 'tbl_rummy_table_user.table_id=tbl_rummy_table.id');
    ...
    public function MakeWinner($game_id, $win_amount, $user_id, $comission)
    {
        ...
        if($user_winning_amt>0){
            log_statement($user_id, RMY, $user_winning_amt,$game_id, $admin_winning_amt);
        }
        return true;
    }
    ...
    public function Gethistory($postData=null)
    {
        ...
        $totalRecords = $this->db->get()->num_rows();
         ...
         if ($min != "" && $max != "") {
            $this->db->where('DATE(tbl_rummy.added_date) >=', $min);
            $this->db->where('DATE(tbl_rummy.added_date) <=', $max);
        }
    ...
    // {
    //     $this->db->select('tbl_rummy.id');
    //     $this->db->from('tbl_rummy');
    //     $this->db->join('tbl_rummy_table_user','tbl_rummy.table_id');
    //     $this->db->where('isDeleted', false);
    //     $this->db->where('winner_id', 0);
