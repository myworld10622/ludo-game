...
class RummyPool_model extends MY_Model
{
    ...
    {
        $this->db->select('tbl_rummy_pool_table.*, tbl_users.name');
        $this->db->from('tbl_rummy_pool_table');
        $this->db->join('tbl_users', 'tbl_users.id = tbl_rummy_pool_table.winner_id', 'left');
        if ($startDate !== null && $endDate !== null) {
            $startDate = date('Y-m-d 00:00:00', strtotime($startDate));
            $endDate = date('Y-m-d 23:59:00', strtotime($endDate));
            $this->db->where('tbl_rummy_pool_table.added_date >=', $startDate); // Prefix 'added_date' with 'tbl_rummy.'
            $this->db->where('tbl_rummy_pool_table.added_date <=', $endDate); // Prefix 'added_date' with 'tbl_rummy.'
            
        }
        $this->db->where('tbl_rummy_pool_table.winner_id !=', 0);
        $this->db->order_by('tbl_rummy_pool_table.id', 'DESC');
        $this->db->limit(10);
        $Query = $this->db->get();
        return $Query->result();
    }
    
    public function getTableMaster($boot_value='',$no_of_players='',$id='')
    {
        ...
        $this->db->join('tbl_rummy_pool_table', 'tbl_rummy_pool_table_master.boot_value=tbl_rummy_pool_table.boot_value AND tbl_rummy_pool_table.isDeleted=0  AND `tbl_rummy_pool_table`.`no_of_players` = '.$no_of_players, 'left');
    ...
    {
        $this->db->select('tbl_rummy_pool_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type,tbl_users.fcm,tbl_rummy_pool_table.no_of_players,tbl_rummy_pool_table.boot_value');
        $this->db->from('tbl_rummy_pool_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_pool_table_user.user_id=tbl_users.id');
        $this->db->join('tbl_rummy_pool_table', 'tbl_rummy_pool_table_user.table_id=tbl_rummy_pool_table.id');
    ...
    public function MakeWinner($game_id, $amount, $user_id, $admin_winning_amt=0)
    {
        ...
        log_statement ($user_id, RMY_POOL, $amount,
    ...
    public function GetTablePoints($table_id)
    {
        ...
        // $this->db->order_by('tbl_rummy_pool_log.user_id', 'ASC');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        // exit;
        return $Query->result();
