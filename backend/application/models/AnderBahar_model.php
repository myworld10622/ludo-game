...
class AnderBahar_model extends MY_Model
{
    ...
    function Gethistory($postData = null)
    {
        ...
        // $this->db->where('tbl_seven_up.isDeleted', false);
             if ($min != "" && $max != "") {
            $this->db->where('DATE(tbl_ander_baher.added_date) >=', $min);
            $this->db->where('DATE(tbl_ander_baher.added_date) <=', $max);
        }
        $this->db->order_by('tbl_ander_baher.id', 'asc');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_ander_baher.added_date', $searchValue, 'after');
            $this->db->like('tbl_ander_baher.total_users', $searchValue, 'after');
            // $this->db->like('tbl_ander_baher.id', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.total_amount', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.admin_profit', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.winning_amount', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.user_amount', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.comission_amount', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.random', $searchValue, 'after');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_ander_baher.added_date', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.total_users', $searchValue, 'after');
            //$this->db->like('tbl_ander_baher.id', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.total_amount', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.admin_profit', $searchValue, 'after');
            $this->db->or_like('tbl_ander_baher.winning_amount', $searchValue, 'after');
     ...
     $data[] = array(
    "id" => $i,
    "total_users" => '<a href="' . base_url('backend/AnderBahar/ander_baher_bet/' . $record->id) . '">' . $record->total_users . '</a>',
    ...
    {
        $this->db->set('ander_bahar_min_bet', $amount);
        $this->db->where('id', 1);
        $this->db->update('tbl_setting');
    }
}