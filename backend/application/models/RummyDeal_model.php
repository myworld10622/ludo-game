...
{
    public function getTableMaster($boot_value='',$id='')
    {
    ...
    {
        $sql = "SELECT * FROM ( SELECT 1 AS mycolumn UNION SELECT 2) a WHERE mycolumn NOT in ( SELECT seat_position FROM `tbl_rummy_deal_table_user` WHERE table_id=" . $TableId . " AND isDeleted=0 ) LIMIT 1";
    ...
    {
        $this->db->select('tbl_rummy_deal_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type,tbl_users.fcm,tbl_rummy_deal_table.boot_value');
        $this->db->from('tbl_rummy_deal_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_deal_table_user.user_id=tbl_users.id');
        $this->db->join('tbl_rummy_deal_table', 'tbl_rummy_deal_table_user.table_id=tbl_rummy_deal_table.id');
    ...
    public function Chaal($game_id, $amount, $user_id)
    {
        ...
        $this->db->insert('tbl_rummy_deal_log', $data);
        log_statement ($user_id, RMY_DEAL, -$amount,
        $game_id,0);
        return true;
    }
    ...
    public function GetTablePoints($table_id)
    {
        ...
        $this->db->from('tbl_rummy_deal_log');
        $this->db->join('tbl_rummy_deal', 'tbl_rummy_deal_log.game_id=tbl_rummy_deal.id','LEFT');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_deal_log.user_id','LEFT');
        $this->db->where('tbl_rummy_deal.table_id', $table_id);
        $this->db->where('tbl_rummy_deal_log.action', 3);
        $this->db->order_by('tbl_rummy_deal_log.id', 'ASC');
        // $this->db->order_by('tbl_rummy_deal_log.user_id', 'ASC');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        // exit;
        return $Query->result();
    ...
    {
        $this->db->select('tbl_rummy_deal_table.*,tbl_users.name');
        $this->db->from('tbl_rummy_deal_table');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_deal_table.winner_id', 'left');
        $this->db->order_by('tbl_rummy_deal_table.added_date', 'DESC');
        $this->db->limit(10);
        if ($startDate !== null && $endDate !== null) {
            $startDate = date('Y-m-d 00:00:00', strtotime($startDate));
            $endDate = date('Y-m-d 23:59:00', strtotime($endDate));
            $this->db->where('tbl_rummy_deal_table.added_date >=', $startDate); // Prefix 'added_date' with 'tbl_rummy.'
            $this->db->where('tbl_rummy_deal_table.added_date <=', $endDate); // Prefix 'added_date' with 'tbl_rummy.'
        }
        $Query = $this->db->get();
        // echo $this->db->last_query();
        // die();
