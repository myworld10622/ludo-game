...
class Lottery_model extends MY_Model
{
    public function getRoom($RoomId = '', $user_id = '')
    {
        ...
        if ($user_id) {
            $this->db->set('lottery_id', $RoomId); //value that used to update column
    ...
    {
        $this->db->set('lottery_id', ''); //value that used to update column
        $this->db->where('id', $user_id); //which row want to upgrade
        $this->db->update('tbl_users');  //table name
    ...
    {
        $Query = $this->db->query('SELECT COUNT(`id`) as online FROM `tbl_lottery_bet` WHERE `lottery_id` = (SELECT `id` FROM `tbl_lottery` WHERE `room_id`=' . $RoomId . ' ORDER BY `id` DESC LIMIT 1)');
        return $Query->row()->online;
    }

    public function getRoomOnlineUser($RoomId)
    {
        $Query = $this->db->query('SELECT * FROM `tbl_users`  WHERE lottery_id = ' . $RoomId);
        return $Query->result();
    }
    ...
    {
        $this->db->from('tbl_lottery_map');
        $this->db->where('lottery_id', $game_id);
        $Query = $this->db->get();
        // echo $this->db->last_query();
        return $Query->result();
    }

    public function CreateMap($lottery_id, $card)
    {
        $ander_data = ['lottery_id' => $lottery_id, 'card' => $card, 'added_date' => date('Y-m-d H:i:s')];
        $this->db->insert('tbl_lottery_map', $ander_data);
        return $this->db->insert_id();
    }

    public function MapCount($lottery_id, $card)
    {
        $this->db->from('tbl_lottery_map');
    ...
    {
        $this->db->insert('tbl_lottery_bet', $bet_data);
        return $this->db->insert_id();
    }

    public function DeleteBet($bet_id, $user_id, $game_id)
    {
        return $this->db->where('lottery_id', $game_id)->where('user_id', $user_id)->where('id', $bet_id)->delete('tbl_lottery_bet');
    }
    ...
    public function Update($data, $game_id)
    {
        ...
        $this->db->update('tbl_lottery', $data);
        $GameId =  $this->db->affected_rows();
        // echo $this->db->last_query();
        return $GameId;
    }

    public function ViewBet($user_id = '', $lottery_id = '', $bet = '', $bet_id = '', $limit = '')
    {
        // echo $bet;
        $this->db->from('tbl_lottery_bet');
    ...
    public function TotalBetAmount($lottery_id, $bet = '')
    {
    ...
    public function MakeWinner($user_id, $bet_id, $amount, $comission, $game_id)
    {
        ...
        $this->db->where('id', $bet_id);
        $this->db->update('tbl_lottery_bet');
    ...
    public function Create($room_id)
    {
        ...
        $this->db->insert('tbl_lottery', $ander_data);
        return $this->db->insert_id();
    }
    ...
    {
        $que = 'SELECT tbl_lottery.id,tbl_lottery.end_datetime as time,SUM(tbl_lottery_bet.winning_amount) as rewards,(SELECT GROUP_CONCAT(`card`) FROM `tbl_lottery_map` WHERE `lottery_id`=tbl_lottery.id GROUP BY `lottery_id`) as type,COUNT(tbl_lottery_bet.id) as winners FROM `tbl_lottery` JOIN tbl_lottery_bet ON tbl_lottery.id=tbl_lottery_bet.lottery_id WHERE tbl_lottery.`winning`=6 AND tbl_lottery.status=1 GROUP BY tbl_lottery.id ORDER BY tbl_lottery.id DESC';
        if (!empty($limit)) {
            $que .= ' LIMIT ' . $limit;
        }
    ...
    public function getJackpotBigWinners($lottery_id)
    {
        $Query = $this->db->query('SELECT tbl_lottery_bet.amount,tbl_lottery_bet.winning_amount,tbl_users.name,tbl_users.profile_pic FROM `tbl_lottery_bet` JOIN tbl_users ON tbl_lottery_bet.user_id=tbl_users.id WHERE tbl_lottery_bet.`lottery_id`=' . $lottery_id . ' ORDER BY winning_amount DESC LIMIT 1');
        return $Query->result();
    }

    public function AllGames()
    {
        $this->db->select('tbl_lottery.*,(select count(id) from tbl_lottery_bet where tbl_lottery.id=tbl_lottery_bet.lottery_id) as total_users');
        $this->db->from('tbl_lottery');
        $this->db->order_by('id', 'DESC');
        $this->db->limit(10);
        $Query = $this->db->get();
    ...
    {
        $return = false;
        $this->db->set('lottery_random', $this->input->post('type')); //value that used to update column
    ...
    function Gethistory($postData = null)
    {
        ...
        $this->db->order_by('tbl_lottery.id', 'asc');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_lottery.added_date', $searchValue, 'after');
            $this->db->like('tbl_lottery.total_users', $searchValue, 'after');
            // $this->db->like('tbl_lottery.id', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.total_amount', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.admin_profit', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.winning_amount', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.user_amount', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.comission_amount', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.random', $searchValue, 'after');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_lottery.added_date', $searchValue, 'after');
            $this->db->or_like('tbl_lottery.total_users', $searchValue, 'after');
        ...
        if ($min != "" && $max != "") {
            $this->db->where('DATE(tbl_lottery.added_date) >=', $min);
            $this->db->where('DATE(tbl_lottery.added_date) <=', $max);
        }
        ...
        foreach ($records as $record) {
            ...
            $data[] = array(
                "id" => $i,
                "total_users" => '<a href="' . base_url('backend/Lottery/LotteryBet/' . $record->id) . '">' . $record->total_users . '</a>',
                //"user_id"=>$record->id,
