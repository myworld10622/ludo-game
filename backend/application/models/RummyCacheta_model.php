...
class RummyCacheta_model extends MY_Model
{
    public function getActiveTable()
    {
        $this->db->select('tbl_users.rummy_cacheta_table_id,COUNT(tbl_users.id) AS members,tbl_rummy_cacheta_table.private,tbl_rummy_cacheta_table.boot_value');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_cacheta_table', 'tbl_users.rummy_cacheta_table_id=tbl_rummy_cacheta_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        // $this->db->where('tbl_rummy_cacheta_table.private', false);
        $this->db->where('tbl_users.rummy_cacheta_table_id!=', 0);
        $this->db->group_by('tbl_users.rummy_cacheta_table_id');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_users.rummy_cacheta_table_id,COUNT(tbl_users.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_cacheta_table', 'tbl_users.rummy_cacheta_table_id=tbl_rummy_cacheta_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        $this->db->where('tbl_rummy_cacheta_table.private', false);
    ...
    public function getTableMaster($boot_value='',$no_of_players='')
    {
        ...
        $this->db->select('tbl_rummy_cacheta_table_master.*,COUNT(tbl_users.id) AS online_members,tbl_rummy_cacheta_table.no_of_players');
        $this->db->from('tbl_rummy_cacheta_table_master');
        $this->db->join('tbl_rummy_cacheta_table', 'tbl_rummy_cacheta_table_master.boot_value=tbl_rummy_cacheta_table.boot_value AND tbl_rummy_cacheta_table.isDeleted=0 AND `tbl_rummy_cacheta_table`.`no_of_players` = '.$no_of_players, 'left');
        $this->db->join('tbl_users', 'tbl_users.rummy_cacheta_table_id=tbl_rummy_cacheta_table.id AND tbl_users.isDeleted=0', 'left');
        // $this->db->where('', false);
        // $this->db->where('tbl_users.table_id!=', 0);
        if (!empty($boot_value)) {
            $this->db->where('tbl_rummy_cacheta_table_master.boot_value', $boot_value);
        }
        // if (!empty($no_of_players)) {
        //     $this->db->where('tbl_rummy_cacheta_table.no_of_players', $no_of_players);
        // }
        $this->db->where('tbl_rummy_cacheta_table_master.isDeleted', 0);
        $this->db->group_by('tbl_rummy_cacheta_table_master.boot_value');
        $this->db->order_by('tbl_rummy_cacheta_table_master.boot_value');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        return $Query->result();
    }
    ...
    public function getCustomizeActiveTable($boot_value,$no_of_players='')
    {
        ...
        $this->db->where('tbl_rummy_cacheta_table.boot_value', $boot_value);
        if(!empty($no_of_players)){
            $this->db->where('tbl_rummy_cacheta_table.no_of_players', $no_of_players);
        }
        $this->db->where('tbl_users.rummy_cacheta_table_id!=', 0);
    ...
    {
        $this->db->select('rummy_cacheta_table_id');
        $this->db->from('tbl_users');
        $this->db->where('isDeleted', false);
        $this->db->where('rummy_cacheta_table_id', $TableId);
        $Query = $this->db->get();
        return $Query->row();
    }
    ...
    {
        $this->db->from('tbl_rummy_cacheta_table');
        $this->db->where('isDeleted', false);
    ...
    {
        $sql = "SELECT * FROM ( SELECT 1 AS mycolumn UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 ) a WHERE mycolumn NOT in ( SELECT seat_position FROM `tbl_rummy_cacheta_table_user` WHERE table_id=" . $TableId . " AND isDeleted=0 ) LIMIT 1";
    ...
    {
        $this->db->select('tbl_rummy_cacheta_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type,tbl_rummy_cacheta_table.no_of_players');
        $this->db->from('tbl_rummy_cacheta_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_cacheta_table_user.user_id=tbl_users.id');
        $this->db->join('tbl_rummy_cacheta_table', 'tbl_rummy_cacheta_table_user.table_id=tbl_rummy_cacheta_table.id');
        $this->db->where('tbl_rummy_cacheta_table_user.isDeleted', false);
        $this->db->where('tbl_rummy_cacheta_table_user.table_id', $TableId);
        $this->db->order_by('tbl_rummy_cacheta_table_user.seat_position', 'asc');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    {
        $this->db->from('tbl_rummy_cacheta_card');
    ...
    public function getGameBot($game_id)
    {
        ...
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_cacheta_card', 'tbl_rummy_cacheta_card.user_id=tbl_users.id');
        $this->db->where('tbl_users.mobile', "");
        $this->db->where('tbl_rummy_cacheta_card.packed', false);
        $this->db->where('tbl_rummy_cacheta_card.game_id', $game_id);
        $Query = $this->db->get();
        return $Query->row();
    }
    ...
    {
        $return = false;
        $this->db->from('tbl_rummy_cacheta_log');
        $this->db->where('user_id', $user_id);
        $this->db->order_by('id', 'DESC');
        $Query = $this->db->get();
    ...
    {
        $this->db->select('tbl_rummy_cacheta_card.*,tbl_users.name,tbl_users.user_type');
        $this->db->from('tbl_rummy_cacheta_card');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_cacheta_card.user_id');
        $this->db->where('tbl_rummy_cacheta_card.game_id', $game_id);
        $this->db->group_by('tbl_rummy_cacheta_card.user_id');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_rummy_cacheta_card.user_id,tbl_rummy_cacheta_card.packed,tbl_users.name');
        $this->db->from('tbl_rummy_cacheta_card');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_cacheta_card.user_id');
        $this->db->where('tbl_rummy_cacheta_card.game_id', $game_id);
        if (!empty($user_id)) {
            $this->db->where('tbl_rummy_cacheta_card.user_id', $user_id);
        }
        $this->db->group_by('tbl_rummy_cacheta_card.user_id');
        $Query = $this->db->get();
        return $Query->result();
    ...
    {
        $this->db->from('tbl_rummy_cacheta');
        $this->db->where('isDeleted', false);
    ...
    public function getMyCards($game_id, $user_id, $card='')
    {
        ...
        // $this->db->update('tbl_rummy_cacheta_card');  //table name
    ...
    public function GetGameCard($game_id)
    {
        ...
        $this->db->where('`cards` NOT IN (SELECT `joker` FROM `tbl_rummy_cacheta` WHERE `id`='.$game_id.')', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_cacheta_card` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_cacheta_card_drop` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
    ...
    public function SwapCards($user_id, $game_id, $my_card, $new_card)
    {
        ...
        $this->db->update('tbl_rummy_cacheta_card', $data, $where);
        ...
        $this->db->insert('tbl_rummy_cacheta_card', $table_user_data);
    ...
    {
        $this->db->insert('tbl_rummy_cacheta', $data);
    ...
    {
        $this->db->insert('tbl_rummy_cacheta_table', $data);
        $TableId =  $this->db->insert_id();
    ...
    {
        $this->db->insert('tbl_rummy_cacheta_table_user', $data);
        $TableId =  $this->db->insert_id();

        $this->db->set('rummy_cacheta_table_id', $data['table_id']); //value that used to update column
    ...
    public function RemoveTableUser($data)
    {
        ...
        $this->db->update('tbl_rummy_cacheta_table_user');  //table name

        $this->db->set('rummy_cacheta_table_id', 0); //value that used to update column
        $this->db->where('id', $data['user_id']); //which row want to upgrade
        $this->db->update('tbl_users');  //table name
    ...
    public function PackGame($user_id, $game_id, $timeout = 0, $json = '', $amount = 0, $percent = 0)
    {
        ...
        $this->db->update('tbl_rummy_cacheta');
        ...
        $this->db->insert('tbl_rummy_cacheta_log', $data);
        return true;
    }
    ...
    public function MakeWinner($game_id, $win_amount, $user_id, $comission)
    {
        ...
        log_statement ($user_id, RMYCACHETA, $user_winning_amt,
    ...
    {
        $this->db->insert('tbl_rummy_cacheta_card', $data);
        $TableId =  $this->db->insert_id();
        // echo $this->db->last_query();
        return $TableId;
    }
    ...
    public function DropGameCards($where, $json='', $timeout=0)
    {
        ...
        $this->db->insert('tbl_rummy_cacheta_card_drop', $where);
    ...
    {
        $this->db->where('id', $game_id);
        $this->db->update('tbl_rummy_cacheta', $data);
    ...
    {
        $this->db->select('tbl_rummy_cacheta.*');
        $this->db->from('tbl_rummy_cacheta');
        $this->db->where('isDeleted', false);
        $this->db->where('tbl_rummy_cacheta.id', $id);
    ...
    public function Delete($id)
    {
        ...
        $return = $this->db->update('tbl_rummy_cacheta');  //table name
    ...
    public function DeleteTable($id)
    {
        ...
        $this->db->set('isDeleted', true); //value that used to update column
        $this->db->where('id', $id); //which row want to upgrade
        $return = $this->db->update('tbl_rummy_cacheta_table');  //table name

        $this->db->set('rummy_cacheta_table_id', 0); //value that used to update column
        $this->db->where('rummy_cacheta_table_id', $id); //which row want to upgrade
    ...
    {
        $Query = $this->db->select('SUM(tbl_rummy_cacheta.amount) as Total_Win,tbl_rummy_cacheta.winner_id,tbl_users.name,tbl_users.profile_pic')
            ->from('tbl_rummy_cacheta')
            ->join('tbl_users', 'tbl_users.id=tbl_rummy_cacheta.winner_id')
            ->where('tbl_rummy_cacheta.winner_id!=', 0)
            ->group_by('tbl_rummy_cacheta.winner_id')
            ->order_by('SUM(tbl_rummy_cacheta.amount)', 'desc')
            ->limit(50)
            ->get();
        // echo $this->db->last_query();
        // exit;
    ...
    {
        $this->db->select('card');
        $this->db->from('tbl_rummy_cacheta_card_drop');
        $this->db->where('game_id', $game_id);
        $this->db->where('isDeleted', false);
        $this->db->limit(1);
        $this->db->order_by('id', 'DESC');
    ...
    public function GetAndDeleteGameDropCard($game_id)
    {
        ...
        if ($result) {
            ...
            $data = ['isDeleted' => 1, 'updated_date' => date('Y-m-d H:i:s')];
            $this->db->update('tbl_rummy_cacheta_card_drop', $data, $where);
        }
        // echo $this->db->last_query();
        // exit;
        return $result;
    ...
    public function ChangeCard($game_id, $User_id, $Position, $Card)
    {
        ...
        $this->db->where('game_id', $game_id);
        $this->db->where('user_id', $User_id);
        $Update = $this->db->update('tbl_rummy_cacheta_card', $data);
        if ($Update) {
    ...
    {
        $this->db->select('tbl_rummy_cacheta.*,tbl_users.name');
        $this->db->from('tbl_rummy_cacheta');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_cacheta.winner_id', 'left');
        $this->db->order_by('tbl_rummy_cacheta.id', 'DESC');
        $this->db->limit(10);
        $Query = $this->db->get();
        // echo $this->db->last_query();
        // die();
    ...
    public function Gethistory($postData=null)
    {
        ...
        $this->db->select('tbl_rummy_cacheta.*, tbl_users.name, tbl_rummy_cacheta_log.game_id, tbl_table.private');
        $this->db->from('tbl_rummy_cacheta');
        $this->db->join('tbl_users', 'tbl_users.id = tbl_rummy_cacheta.winner_id', 'left');
        $this->db->join('tbl_rummy_cacheta_log', 'tbl_rummy_cacheta_log.game_id = tbl_rummy_cacheta.id', 'left');
        $this->db->join('tbl_table', 'tbl_table.id = tbl_rummy_cacheta.table_id', 'left');
        $this->db->group_by('tbl_rummy_cacheta_log.game_id');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_rummy_cacheta.added_date', $searchValue, 'after');
            $this->db->like('tbl_users.name', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_cacheta.winner_id', $searchValue, 'after');
            $this->db->or_like('tbl_table.private', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_cacheta_log.game_id', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_cacheta.amount', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_cacheta.user_winning_amt', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_cacheta.admin_winning_amt', $searchValue, 'after');
            //$this->db->or_like('tbl_rummy_cacheta.comission_amount', $searchValue, 'after');
        ...
        $this->db->join('tbl_table', 'tbl_table.id = tbl_rummy_cacheta.table_id', 'left');
        $this->db->group_by('tbl_rummy_cacheta_log.game_id');
        $this->db->order_by('tbl_rummy_cacheta.added_date', 'DESC');
        // Add any necessary conditions or additional joins here
        $this->db->order_by('tbl_rummy_cacheta.id', 'desc');
