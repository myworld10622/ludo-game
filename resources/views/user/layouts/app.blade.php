...
<head>
    ...
    <title>@yield('title', 'User Panel') — RoxLudo</title>
...
<div class="shell" id="userPanelShell">
    ...
    <aside class="sidebar">
        ...
        <a class="nav-link {{ request()->routeIs('panel.index') ? 'active' : '' }}" href="{{ route('panel.index') }}">
            <span class="nav-icon">🏠</span> Dashboard
        </a>
        @if (!empty($panelPermissions['manage_tournaments']))
            <a class="nav-link {{ request()->routeIs('panel.tournaments.*') ? 'active' : '' }}" href="{{ route('panel.tournaments.index') }}">
                <span class="nav-icon">🏆</span> Tournaments
            </a>
        @endif
        @if (!empty($panelPermissions['view_match_monitor']))
            <a class="nav-link {{ request()->routeIs('panel.matches.*') ? 'active' : '' }}" href="{{ route('panel.matches.index') }}">
                <span class="nav-icon">⚡</span> Match Monitor
            </a>
        @endif
        <a class="nav-link {{ request()->routeIs('panel.support.*') ? 'active' : '' }}" href="{{ route('panel.support.index') }}">
            <span class="nav-icon">💬</span> Support Chat
        </a>
        <div class="sidebar-section-label">Resources</div>
        <a class="nav-link guide-link" href="{{ route('tournament.guide') }}" target="_blank">
    ...
    <main class="content">
        <div class="topbar">
            <div class="topbar-main">
                ...
                <div>
                    <div class="topbar-heading">@yield('heading', 'User Panel')</div>
                    <div class="topbar-sub">@yield('subheading', 'Manage your tournaments and matches')</div>
                </div>
            </div>
        ...
        @if (session('error'))<div class="error-list">{{ session('error') }}</div>@endif
        @if ($errors->any())
            <div class="error-list">@foreach ($errors->all() as $error)<div>{{ $error }}</div>@endforeach</div>
        @endif
        @yield('content')
    </main>
</div>
