...
@section('title', $mode === 'edit' ? 'Edit Distributor Gateway' : 'Add Distributor Gateway')
@section('heading', $mode === 'edit' ? 'Edit Distributor Gateway' : 'Add Distributor Gateway')
@section('subheading', 'Assign gateway number for a distributor')

@section('content')
<div class="panel">
    <form method="POST" action="{{ $mode === 'edit' ? route('admin.gateways.distributor.update', $row->id ?? 0) : route('admin.gateways.distributor.store') }}">
        @csrf
        @if ($mode === 'edit')
            @method('PUT')
        @endif
        ...
        <div class="mobile-actions" style="margin-top: 16px;">
            ...
            <a class="btn btn-secondary" href="{{ route('admin.gateways.distributor.index') }}">Cancel</a>
        </div>
    </form>
</div>
@endsection
