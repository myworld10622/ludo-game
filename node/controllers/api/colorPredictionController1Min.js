...
const colorPredictionService = require('../../services/colorPredictionService1Min');
...
class ColorPredictionController {
    ...
    async placeBet(req, res) {
        try {
            ...
            userWallet.minusUserWallet(user_id, amount, GAMES.cp1, betData);
            const responseData = {
                bet_id: betData.id,
    ...
    async makeWinner(userId, betId, amount, comission, gameId) {
        try {
            ...
            // Add to Wallet
            userWallet.plusUserWallet(userId, betId, userWinningAmount, adminComissionAmount, GAMES.cp1, dragonTigerBet);
        } catch (error) {
            console.log(error);
        }
    }
                ...
                for (let index = 0; index < rooms.length; index++) {
                    ...
                    if (gameData[0].status === 0) {
                        ...
                        if ((addedDateTimestamp + COLOR_PREDICTION_FOR_BET) <= currentTime) {
                            ...
                            const setting = await adminService.setting(["color_prediction_1_min_random", "admin_commission", "admin_coin", "distribute_precent"]);
                            const random = setting.color_prediction_1_min_random;
                            let minNumber;
                            ...
                            // const setting = await adminService.setting(["color_prediction_1_min_random", "admin_commission", "admin_coin"]);
                            // const random = setting.color_prediction_1_min_random;
                            // console.log('pradeep', random)
                            // let minNumber;
                            // // Logic for winning
                            // if (random == COLOR_PREDICTIONS_BETS.random) {
                            //     const numArray = ['ZERO', 'ONE', 'TWO', 'THREE', 'FOUR', 'FIVE', 'SIX', 'SEVEN', 'EIGHT', 'NINE'];
                            ...
                            if (updatePayload.admin_profit != 0) {
                                userWallet.directAdminProfitStatement(GAMES.cp1, updatePayload.admin_profit, gameData[0].id);
                            }
