...
const colorPredictionService = require('../../services/colorPredictionService5Min');
...
const { getAmountByPercentage, getRoundNumber, getRandomFromFromArray } = require('../../utils/util');
...
class ColorPredictionController {
    ...
    async placeBet(req, res) {
        try {
            ...
            userWallet.minusUserWallet(user_id, amount, GAMES.cp5, betData);
            const responseData = {
                bet_id: betData.id,
    ...
    async makeWinner(userId, betId, amount, comission, gameId) {
        try {
            ...
            // Add to Wallet
            userWallet.plusUserWallet(userId, betId, userWinningAmount, adminComissionAmount, GAMES.cp5, dragonTigerBet);
        } catch (error) {
            console.log(error);
        }
    }
                ...
                for (let index = 0; index < rooms.length; index++) {
                    ...
                    if (gameData[0].status === 0) {
                        ...
                        if ((addedDateTimestamp + COLOR_PREDICTION_FOR_BET) <= currentTime) {
                            ...
                            const setting = await adminService.setting(["color_prediction_5_min_random", "admin_commission", "admin_coin", "distribute_precent"]);
                            const random = setting.color_prediction_5_min_random;
                            let minNumber;
                            ...
                            // const setting = await adminService.setting(["color_prediction_5_min_random", "admin_commission", "admin_coin"]);
                            // const random = setting.color_prediction_5_min_random;
                            // let minNumber;
                            // // Logic for winning
                            // if (random == COLOR_PREDICTIONS_BETS.random) {
                            ...
                            if (updatePayload.admin_profit != 0) {
                                userWallet.directAdminProfitStatement(GAMES.cp5, updatePayload.admin_profit, gameData[0].id);
                            }
