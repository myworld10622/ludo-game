const { HTTP_NOT_ACCEPTABLE, HTTP_SWITCH_PROTOCOL, HTTP_WIN, HTTP_LOOSER, GAMES, JHANDI_MUNDA_FOR_BET, JHANDI_MUNDA_TIME_FOR_START_NEW_GAME, JHANDI_MUNDA_WINNING_PRICE } = require('../../constants');
const adminService = require('../../services/adminService');
const jhandiMundaService = require('../../services/jhandiMundaService');
...
class JhandiMundaController {
    ...
    async placeBet(req, res) {
        try {
            ...
            // game
            const game = await jhandiMundaService.getById(game_id);
            if (!game) {
            ...
            const payload = {
                jhandi_munda_id: game_id,
                user_id,
                bet,
                amount
            }

            const betData = await jhandiMundaService.createBet(payload);
            if (!betData) {
                return errorResponse(res, "Something Wents Wrong", HTTP_NOT_ACCEPTABLE);
            }
            // Not wait for calculation of wallet
            userWallet.minusUserWallet(user_id, amount, GAMES.jhandiMunda, betData);
            const responseData = {
                bet_id: betData.id,
    ...
    async getResult(req, res) {
        try {
            ...
            const betData = await jhandiMundaService.viewBet(user_id, game_id);
            let winAmount = 0;
            let betAmount = 0;
    ...
    async walletHistory(req, res) {
        try {
            ...
            const walletHistory = await jhandiMundaService.walletHistory(user_id);
            // const setting = await adminService.setting(["min_redeem"]);
            const responsePayload = {
                JhandiMundalog: walletHistory
            }
    ...
    async makeWinner(userId, betId, amount, comission, gameId) {
        try {
            ...
            // Update bet
            jhandiMundaService.updateBet(betId, gameBetPayload);
            ...
            jhandiMundaService.update(gameId, gamePayload);

            // Get Bet to check amount deducted from which wallet
            const gameBet = await jhandiMundaService.getBetById(betId, ["id", "minus_unutilized_wallet", "minus_winning_wallet", "minus_bonus_wallet"]);
            // Add to Wallet
            userWallet.plusUserWallet(userId, betId, userWinningAmount, adminComissionAmount, GAMES.jhandiMunda, gameBet);
        } catch (error) {
            console.log(error);
        }
    }
    ...
    async createGame() {
        try {
            const rooms = await jhandiMundaService.getRooms("", "", ["id"]);
            if (rooms && Array.isArray(rooms) && rooms.length > 0) {
                for (let index = 0; index < rooms.length; index++) {
                    const room = rooms[index];
                    const gameData = await jhandiMundaService.getActiveGameOnTable(room.id, ["id", "status"]);
                    ...
                    if ((Array.isArray(gameData) && gameData.length === 0) || (gameData.length > 0 && gameData[0].status === 1)) {
                        jhandiMundaService.create({ room_id: room.id });
                        console.log('Jhandi Munda Game Created Successfully');
                        return;
                    }
                }
            } else {
            ...
            if (rooms && Array.isArray(rooms) && rooms.length > 0) {
                for (let index = 0; index < rooms.length; index++) {
                    ...
                    if (gameData[0].status === 0) {
                        ...
                        const setting = await adminService.setting(["jhandi_munda_random", "admin_commission", "admin_coin"]);
                        const random = setting.jhandi_munda_random;
                        const admin_coin = setting.admin_coin;

                        const bets = await jhandiMundaService.viewBet('', gameId);
                        const { totalBetAmount, heartBetAmount, spadeBetAmount, diamondBetAmount, clubBetAmount, faceBetAmount, flagBetAmount } = await jhandiMundaService.getBetDataByBets(bets);

                        if (random == 1) {
                            await jhandiMundaService.createMap(gameId, getRandomNumber(1, 6));
                            await jhandiMundaService.createMap(gameId, getRandomNumber(1, 6));
                            await jhandiMundaService.createMap(gameId, getRandomNumber(1, 6));
                            await jhandiMundaService.createMap(gameId, getRandomNumber(1, 6));
                            await jhandiMundaService.createMap(gameId, getRandomNumber(1, 6));
                       ...
                       /* else if (random == 2) {
                                ...
                                if (diceCount > 0) {
                                    ...
                                    for (let ind = 0; ind < (randomDice - 1); ind++) {
                                        switch ((randomDice - ind)) {
                                            case 1:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.oneDice;
                                                break;
                                            case 2:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.twoDice;
                                                break;
                                            case 3:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.threeDice;
                                                break;
                                            case 4:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.fourDice;
                                                break;
                                            case 5:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.fiveDice;
                                                break;
                                            case 6:
                                                betMultiply = JHANDI_MUNDA_WINNING_PRICE.sixDice;
                                                break;
                                            default:
                                                break;
                                        }
                                    ...
                                    for (let i = 0; i < dice; i++) {
                                        await jhandiMundaService.createMap(gameId, getRandomNumber(1, k));
                                    }
                            ...
                            for (const [key, value] of sortedArr) {
                                if (diceCount > 0) {
                                    ...
                                    let dice;
                                    if (remainingBalance > (value * JHANDI_MUNDA_WINNING_PRICE.spade)) {
                                        
                                        const twoDice = diceCount >= 2 ? 2 : diceCount;
                                        const threeDice = diceCount >= 3 ? 3 : diceCount;
                                        dice = (value * JHANDI_MUNDA_WINNING_PRICE.spade) === 0 ? getRandomNumber(1, threeDice) : twoDice;

                                        remainingBalance -= (value * JHANDI_MUNDA_WINNING_PRICE.spade);
                                        console.log("Remaining Balance Here", twoDice, threeDice, remainingBalance, value * JHANDI_MUNDA_WINNING_PRICE.spade, dice)
                                    } else {
                        ...
                        for (let i = 1; i <= 6; i++) {
                            ...
                            const count = await jhandiMundaService.mapCount(gameData[0].id, i);
                            ...
                            if (count > 0) {
                                ...
                                switch (count) {
                                    case 1:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.oneDice;
                                        break;
                                    case 2:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.twoDice;
                                        break;
                                    case 3:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.threeDice;
                                        break;
                                    case 4:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.fourDice;
                                        break;
                                    case 5:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.fiveDice;
                                        break;
                                    case 6:
                                        multiply = JHANDI_MUNDA_WINNING_PRICE.sixDice;
                                        break;
                                    default:
                                        break;
                                }
                                ...
                                if (multiply > 0) {
                                    ...
                                    const winnerBets = await jhandiMundaService.viewBet('', gameId, i);
                        ...
                        const endDateTime = dateFormat.asString('yyyy-MM-dd hh:mm:ss', new Date(now.getTime() + JHANDI_MUNDA_TIME_FOR_START_NEW_GAME * 1000));
                        ...
                        await jhandiMundaService.update(gameId, updatePayload);
                        // If admin profit is in positive or nagative then log this
                        if (updatePayload.admin_profit != 0) {
                            userWallet.directAdminProfitStatement(GAMES.jhandiMunda, updatePayload.admin_profit, gameId);
                        }
    ...
    async getActiveGameSocket(roomId) {
        try {
            ...
            const gameData = await jhandiMundaService.getActiveGameOnTable(roomId, ["id", "status", "added_date", "room_id", "winning", "end_datetime", "updated_date", "random_amount"]);
            if ((Array.isArray(gameData) && gameData.length > 0)) {
                let gameCards = [];
                const gameId = gameData[0].id;
                if (gameData[0].status) {
                    gameCards = await jhandiMundaService.getGameCards(gameId);
                }
                ...
                // Remaining Time 
                const timeRemaining = (addedDatetimeSec + JHANDI_MUNDA_FOR_BET) - currentTimeSec;
                ...
                // Get Online Users
                const online = await jhandiMundaService.getRoomOnlineUsers(roomId);
                const onlineUsers = await getByConditions({ jhandi_munda_id: roomId });
                // Get Bets for games
                const bets = await jhandiMundaService.viewBet('', gameId);
                // const { dragonBetAmount, tigerBetAmount, tieBetAmount } = await jhandiMundaService.getBetDataByBets(bets, 0);
                const { heartBetAmount, spadeBetAmount, diamondBetAmount, clubBetAmount, faceBetAmount, flagBetAmount } = await jhandiMundaService.getBetDataByBets(bets);

                const lastWinnings = await jhandiMundaService.lastWinningBet(roomId, 15);
                ...
                jhandiMundaService.update(gameId, { random_amount: db.sequelize.literal(`random_amount + ${responsePayload.heart_amount}`) });
...
module.exports = new JhandiMundaController();