...
module.exports = function (jackpot_socket, request, BASE_URL) {
    ...
    jackpot_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/jackpot_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("jackpot_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/jackpot_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("jackpot_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/jackpot/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    // console.log("jackpot_status", "status");
                    jackpot_socket.emit('jackpot_status', response.body);
