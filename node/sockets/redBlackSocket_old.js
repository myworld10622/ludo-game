...
module.exports = function (red_black_socket, request, BASE_URL) {
    ...
    red_black_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/red_black_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("red_black_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/red_black_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("red_black_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/redBlack/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    // console.log("red_black_status", "status");
                    red_black_socket.emit('red_black_status', response.body);
