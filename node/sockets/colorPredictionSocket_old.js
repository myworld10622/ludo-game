...
module.exports = function (color_prediction_socket, request, BASE_URL) {
    ...
    color_prediction_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/color_prediction_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("color_prediction_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/color_prediction_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("color_prediction_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/colorPrediction/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    // console.log("color_prediction_status", "status");
                    color_prediction_socket.emit('color_prediction_status', response.body);
                ...
                if (user_count > 0 && !game_created) {
                    ...
                    setTimeout(function () {
                        ...
                        //     // console.log(response.body);
                        //     // console.log("color_prediction_status", "status");
                        //     console.info("getProtocol", response.socket.getProtocol());
                        //     color_prediction_socket.emit('color_prediction_status', response.body);
