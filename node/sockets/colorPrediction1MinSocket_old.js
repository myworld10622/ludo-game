...
module.exports = function (color_prediction_1min_socket, request, BASE_URL) {
    ...
    color_prediction_1min_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/color_prediction_1_min_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                console.log("color_prediction_1_min_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/color_prediction_1_min_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("color_prediction_1min_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/colorPrediction1Min/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    console.log("color_prediction_1min_status", "status");
                    color_prediction_1min_socket.emit('color_prediction_1min_status', response.body);
