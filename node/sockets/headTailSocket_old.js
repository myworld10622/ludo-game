...
module.exports = function (head_tail_socket, request, BASE_URL) {
    ...
    head_tail_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/head_tail_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("head_tail_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/head_tail_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("head_tail_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/headTail/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    // console.log("head_tail_status", "status");
                    head_tail_socket.emit('head_tail_status', response.body);
