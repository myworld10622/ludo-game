...
module.exports = function (car_roulette_socket, request, BASE_URL) {
    ...
    car_roulette_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/car_roulette_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("car_roulette_create_socket", response.body);
            });
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': BASE_URL+'/api/cron/car_roulette_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                // console.log("car_roulette_winner_socket", response.body);

                // Send Status
                var options = {
                    'method': 'POST',
                    'url': BASE_URL+'/api/carRoulette/get_active_game_socket',
                    'headers': {
                ...
                request(options, function (error, response) {
                    ...
                    // console.log("car_roulette_status", "status");
                    car_roulette_socket.emit('car_roulette_status', response.body);
