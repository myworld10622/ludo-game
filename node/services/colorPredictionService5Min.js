...
const ColorPrediction = db.ColorPrediction5Min;
const ColorPredictionBet = db.ColorPredictionBet5Min;
const ColorPredictionRoom = db.ColorPredictionRoom5Min;
const ColorPredictionMap = db.ColorPredictionMap5Min;
const Card = db.Card;
    ...
    async walletHistory(userId) {
        try {
            const bets = await ColorPredictionBet.findAll({
                attributes: [
                    '*',
                    'tbl_color_prediction_5_min.room_id'
                    // [db.Sequelize.col('tbl_color_prediction_5_min.room_id'), 'room_id']
                ],
                include: [{
                    ...
                    // where: {
                    //   id: db.Sequelize.col('tbl_color_prediction_5_min_bet.color_prediction_id')
                    // }
                }],
                where: {
                    user_id: userId
    ...
    async myHistory(userId, limit) {
        try {
            const queryOptions = {
                attributes: [
                    '*',
                    'tbl_color_prediction_5_min.status'
                ],
                include: [{
                    model: ColorPrediction,
                    attributes: [],
