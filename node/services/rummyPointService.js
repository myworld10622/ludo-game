...
const { RummyPoint, RummyPointTable, user, RummyPointTableUser, sequelize, RummyPointTableMaster, Sequelize, RummyPointCard, CardRummy, RummyPointLog, RummyPointCardDrop } = require('../models');
...
class RummyPointService {
    async getById(id, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            const game = await RummyPoint.findByPk(id, attributeOptions);
    ...
    async create(data) {
        try {
            return await RummyPoint.create(data);
        } catch (error) {
            console.log(error)
    ...
    async getTable(tableId, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            return await RummyPointTable.findByPk(tableId, attributeOptions);
        } catch (error) {
            console.log(error)
    ...
    async chaalCount(gameId, userId) {
        try {
            return await RummyPointLog.count({
                where: {
                    game_id: gameId,
                    action: 2,
                    user_id: userId,
    ...
    async packGame(userId, gameId, timeout = 0, json = '', amount = 0, percent = 0) {
        try {
            await RummyPointCard.update(
                {
                    packed: 1
                },
                {
            ...
            return await RummyPointLog.create(data);
        } catch (error) {
            console.log(error)
            throw new Error('Error fetching table');
        }
    ...
    async getMyCards(gameId, userId, card = "") {
        try {
            ...
            return await RummyPointCard.findAll({
                attributes: [
                    'id',
                    'card',
    ...
    async getMyCard(gameId, userId, card) {
        try {
            ...
            return await RummyPointCard.findOne({
                attributes: [
                    'id',
                    'card',
                    [sequelize.fn('SUBSTRING', sequelize.col('card'), 1, 2), 'card_group']
    ...
    async getTableByCode(code, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            return await RummyPointTable.findOne({
                where: {
                    private: 2,
                    code,
                },
    ...
    async tableUsers(tableId) {
        try {
            const tableUsers = await RummyPointTableUser.findAll({
                attributes: [
                    ...
                    [sequelize.col('tbl_rummy_table.no_of_players'), 'no_of_players'],
                    [sequelize.col('tbl_rummy_table.boot_value'), 'boot_value']
                ],
                include: [
                    {
                        model: user,
    ...
    async getTableMaster(bootValue="", noOfPlayers) {
        ...
        const rummyTables = await RummyPointTableMaster.findAll({
            attributes: [
                'id',
                'point_value',
                'boot_value',
                [Sequelize.fn('COUNT', Sequelize.col('tbl_rummy_tables.tbl_users.id')), 'online_members'],
                [Sequelize.col('tbl_rummy_tables.no_of_players'), 'no_of_players']
            ],
            include: [
                {
                    model: RummyPointTable,
            ...
            group: [
                'tbl_rummy_table_master.boot_value'
            ],
            // raw: true,
            // nest: true,
            // subQuery: false
    ...
    async getCustomizeActiveTable(tableAmount, noOfPlayers) {
        ...
        const activeTables = await user.findAll({
            attributes: [
                'id',
                'rummy_table_id',
                [Sequelize.fn('COUNT', Sequelize.col('tbl_rummy_table.id')), 'members']
            ],
            include: [
                {
                    model: RummyPointTable,
    ...
    async getActiveGameOnTable(tableId, attributes = []) {
        ...
        return await RummyPoint.findOne(options);
    }
    ...
    async createTable(data) {
        try {
            return await RummyPointTable.create(data);
        } catch (error) {
            console.log(error)
    ...
    async addTableUser(data) {
        try {
            const rummyTableUser = await RummyPointTableUser.create(data);

            if (data.table_id && data.user_id) {
                userService.update(data.user_id, { rummy_table_id: data.table_id })
            }
    ...
    async removeTableUser(data) {
        try {
            await RummyPointTableUser.update(
                {
                    isDeleted: 1
                },
                {
            ...
            if (data.user_id && data.table_id) {
                await userService.update(data.user_id, { rummy_table_id: 0 })
            }
        } catch (error) {
            console.log(error)
    ...
    async gameOnlyUsers(gameId, userId = "") {
        ...
        const results = await RummyPointCard.findAll({
            attributes: ['user_id', 'packed',
                // [sequelize.col('tbl_users.name'), 'name']
            ],
            include: [{
                model: user,
    ...
    async giveGameCards(data) {
        try {
            return await RummyPointCard.create(data);
        } catch (error) {
            console.log(error)
    ...
    async startDropGameCards(payload) {
        ...
        const dropCard = await RummyPointCardDrop.create(payload);
    ...
    async rummyCardDrop(data) {
        try {
            return await RummyPointCardDrop.create(data)
        } catch (error) {
            console.log(error);
    ...
    async checkUsersOnTable(tableId) {
        try {
            return await userService.getSingleByConditions({ rummy_table_id: tableId }, ["id"])
        } catch (error) {
            console.log(error);
            throw new Error("Error while drop Card");
        }
    ...
    async gameAllUsers(gameId) {
        try {
            return await RummyPointCard.findAll({
                attributes: ['tbl_rummy_card.*', 'tbl_user.name', 'tbl_user.user_type'],
                include: [{
                    model: user,
                    as: "tbl_user",
                    attributes: [],
                    // where: { id: sequelize.col('tbl_rummy_card.user_id') }
                }],
    ...
    async dropGameCard(tableData, json, timeout = 0) {
        try {
            ...
            const dropGame = await RummyPointCardDrop.create(tableData);
            ...
            await RummyPointLog.create(gameLogs);
        ...
        try {
            return await CardRummy.findOne({
                where: {
                    cards: {
                        [Op.notIn]: [
                            sequelize.literal(`(SELECT joker FROM tbl_rummy WHERE id = ${gameId} limit 1)`),
                            sequelize.literal(`(SELECT card FROM tbl_rummy_card WHERE game_id = ${gameId} AND isDeleted = 0  limit 1)`),
                            sequelize.literal(`(SELECT card FROM tbl_rummy_card_drop WHERE game_id = ${gameId} AND isDeleted = 0 limit 1)`)
                        ]
                    }
                },
                order: sequelize.random()
    ...
    async getAndDeleteGameDropCard(gameId) {
        try {
            const droppedCard = await RummyPointCardDrop.findOne({
                where: {
                    game_id: gameId
                },
                attributes: ["id", "card"],
            ...
            if (droppedCard) {
                await RummyPointCardDrop.update(
                    {
                        isDeleted: 1
                    },
                    {
    ...
    async declare(data) {
        try {
            await RummyPoint.update(
                {
                    amount: sequelize.literal(`amount + ${data.actual_points}`)
                },
                {
                    where: {
                        id: data.game_id
            ...
            const logData = {
                ...
                points: data.points,
                amount: data.actual_points * -1,
                action: 3,
                json: data.json,
            }

            return await RummyPointLog.create(logData);
        } catch (error) {
    ...
    async gameLog(gameId, limit = '', status = '', user_id = '', timeout = '') {
        try {
            ...
            return await RummyPointLog.findAll(options);
        } catch (error) {
            console.log(error)
    ...
    async gameLogJson(gameId, userId) {
        try {
            return await RummyPointLog.findOne({
                where: {
                    game_id: gameId,
                    user_id: userId
                },
    ...
    async discardedGameCard(gameId) {
        try {
            return await RummyPointCardDrop.findAll({
                where: {
                    game_id: gameId
                },
                order: [["id", "DESC"]]
    ...
    async getGameDropCard(gameId) {
        try {
            return await RummyPointCardDrop.findOne({
                attributes: ["card"],
                where: {
                    game_id: gameId
                },
...
module.exports = new RummyPointService();