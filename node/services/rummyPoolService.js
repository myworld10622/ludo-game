...
const { RummyPool, RummyPoolTable, user, RummyPoolTableUser, sequelize, RummyPoolTableMaster, Sequelize, RummyPoolCard, CardRummy, RummyPoolLog, RummyPoolCardDrop, ShareWallet, RummyDeal } = require('../models');
...
class RummyPoolService {
    async getById(id, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            const game = await RummyPool.findByPk(id, attributeOptions);
    ...
    async getLatestGameOnTable(tableId, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            return await RummyPool.findOne({
                ...attributeOptions,
                where: {
                    table_id: tableId
                }
    ...
    async create(data) {
        try {
            return await RummyPool.create(data);
        } catch (error) {
            console.log(error)
    ...
    async getTable(tableId, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            return await RummyPoolTable.findByPk(tableId, attributeOptions);
        } catch (error) {
            console.log(error)
    ...
    async chaalCount(gameId, userId) {
        try {
            return await RummyPoolLog.count({
                where: {
                    game_id: gameId,
                    action: 2,
                    user_id: userId,
    ...
    async packGame(userId, tableId, gameId, percent = 0) {
        try {
            ...
            await RummyPoolTableUser.update({
            ...
            const tableUser = await RummyPoolTableUser.findOne({
            ...
            const totalPoints = tableUser.total_points;
            ...
            return await RummyPoolLog.create(data);
        } catch (error) {
            console.log(error)
            throw new Error('Error fetching table');
        }
    ...
    async getMyCards(gameId, userId, card = "") {
        try {
            ...
            return await RummyPoolCard.findAll({
                attributes: [
                    'id',
                    'card',
            ...
            console.error('Error querying tbl_rummy_pool_card:', error);
            throw error;
        }
    }
    ...
    async getMyCard(gameId, userId, card) {
        try {
            ...
            return await RummyPoolCard.findOne({
                attributes: [
                    'id',
                    'card',
                    [sequelize.fn('SUBSTRING', sequelize.col('card'), 1, 2), 'card_group']
    ...
    async getTableByCode(code, attributes = []) {
        try {
            const attributeOptions = getAttributes(attributes);
            return await RummyPoolTable.findOne({
                where: {
                    private: 2,
    ...
    async tableUsers(tableId) {
        try {
            const tableUsers = await RummyPoolTableUser.findAll({
                attributes: [
                    ...
                    [sequelize.col('tbl_rummy_pool_table.no_of_players'), 'no_of_players'],
                    [sequelize.col('tbl_rummy_pool_table.boot_value'), 'boot_value']
                ],
                include: [
                    {
                        model: user,
                    ...
                    {
                        model: RummyPoolTable,
                        as: 'tbl_rummy_pool_table',
                        attributes: []
                    }
                ],
                where: {
    ...
    async getTableMaster(bootValue = "", noOfPlayers) {
        const whereConditions = {};
        if (bootValue) {
            whereConditions.boot_value = bootValue;
        }
        const rummyTables = await RummyPoolTableMaster.findAll({
            attributes: [
                'id',
                'boot_value',
                'pool_point',
                [Sequelize.fn('COUNT', Sequelize.col('tbl_rummy_pool_tables.tbl_users.id')), 'online_members'],
                [Sequelize.col('tbl_rummy_pool_tables.no_of_players'), 'no_of_players']
            ],
            include: [
                {
                    model: RummyPoolTable,
                    as: 'tbl_rummy_pool_tables',
                    required: false,
            ...
            group: [
                'tbl_rummy_pool_table_master.boot_value'
            ],
            // raw: true,
            // nest: true,
            // subQuery: false
    ...
    async getCustomizeActiveTable(tableAmount, noOfPlayers) {
        ...
        const activeTables = await user.findAll({
            attributes: [
                'id',
                'rummy_pool_table_id',
                [Sequelize.fn('COUNT', Sequelize.col('tbl_rummy_pool_table.id')), 'members']
            ],
            include: [
                {
                    model: RummyPoolTable,
        ...
        try {
            const sql = `
                ...
                WHERE mycolumn NOT IN (
                    ...
                    FROM tbl_rummy_pool_table_user
    ...
    async getActiveGameOnTable(tableId, attributes = []) {
        ...
        return await RummyPool.findOne(options);
    }
    ...
    async createTable(data) {
        try {
            return await RummyPoolTable.create(data);
        } catch (error) {
            console.log(error)
    ...
    async addTableUser(data) {
        try {
            const rummyTableUser = await RummyPoolTableUser.create(data);

            if (data.table_id && data.user_id) {
                userService.update(data.user_id, { rummy_pool_table_id: data.table_id })
            }
    ...
    async removeTableUser(data) {
        try {
            ...
            if (data.user_id && data.table_id) {
                await userService.update(data.user_id, { rummy_pool_table_id: 0 })
            }
        } catch (error) {
            console.log(error)
    ...
    async gameOnlyUsers(gameId, userId = "") {
        ...
        const results = await RummyPoolCard.findAll({
            ...
            where: whereClause,
            group: ['tbl_rummy_pool_card.user_id']
        });
    ...
    async giveGameCards(data) {
        try {
            return await RummyPoolCard.create(data);
        } catch (error) {
            console.log(error)
    ...
    async startDropGameCards(payload) {
        ...
        const dropCard = await RummyPoolCardDrop.create(payload);
    ...
    async rummyCardDrop(data) {
        try {
            return await RummyPoolCardDrop.create(data)
        } catch (error) {
            console.log(error);
    ...
    async checkUsersOnTable(tableId) {
        try {
            return await userService.getSingleByConditions({ rummy_pool_table_id: tableId }, ["id"])
        } catch (error) {
            console.log(error);
            throw new Error("Error while drop Card");
        }
    ...
    async gameAllUsers(gameId) {
        try {
            return await RummyPoolCard.findAll({
                attributes: ['tbl_rummy_pool_card.*', 'tbl_user.name', 'tbl_user.user_type'],
                include: [{
                    model: user,
                    required: true,
                    as: "tbl_user",
                    attributes: [],
                    // where: { id: sequelize.col('tbl_rummy_pool_card.user_id') }
                }],
    ...
    async dropGameCard(tableData, json, timeout = 0) {
        try {
            ...
            const dropGame = await RummyPoolCardDrop.create(tableData);
            ...
            await RummyPoolLog.create(gameLogs);
                ...
                where: {
                    cards: {
                        [Op.and]: [
                            ...
                            sequelize.literal(`cards NOT IN (SELECT card FROM tbl_rummy_pool_card WHERE game_id = ${gameId} AND isDeleted = 0)`),
                            sequelize.literal(`cards NOT IN (SELECT card FROM tbl_rummy_pool_card_drop WHERE game_id = ${gameId} AND isDeleted = 0)`)
                        ]
                    }
                },
                order: sequelize.random()
    ...
    async getAllGameOnTable(tableId) {
        try {
            return await RummyPool.findAll({
                where: {
                    table_id: tableId
                },
                order: [["id", "DESC"]]
    ...
    async getTablePoints(tableId) {
        try {
            return await RummyPoolLog.findAll({
                attributes: [
                    'game_id',
                    'user_id',
                    'points',
    ...
    async getTotalPoints(table_id, user_id) {
        const result = await RummyPoolTableUser.findOne({
            attributes: ['total_points'],
            where: {
                table_id: table_id,
                user_id: user_id
    ...
    async getAndDeleteGameDropCard(gameId) {
        try {
            const droppedCard = await RummyPoolCardDrop.findOne({
                where: {
                    game_id: gameId
                },
                attributes: ["id", "card"],
            ...
            if (droppedCard) {
                await RummyPoolCardDrop.update(
                    {
                        isDeleted: 1
                    },
                    {
    ...
    async declare(data) {
        try {
            ...
            const userTableData = await RummyPoolTableUser.findOne({
                attributes: ["total_points"],
                where: {
                    table_id: data.table_id,
                    user_id: data.user_id
            ...
            return await RummyPoolLog.create(logData);
        } catch (error) {
    ...
    async gameLog(gameId, limit = '', status = '', user_id = '', timeout = '') {
        try {
            ...
            return await RummyPoolLog.findAll(options);
        } catch (error) {
            console.log(error)
    ...
    async gameLogJson(gameId, userId) {
        try {
            return await RummyPoolLog.findOne({
                where: {
                    game_id: gameId,
                    user_id: userId
                },
    ...
    async discardedGameCard(gameId) {
        try {
            return await RummyPoolCardDrop.findAll({
                where: {
                    game_id: gameId
                },
                order: [["id", "DESC"]]
    ...
    async getGameDropCard(gameId) {
        try {
            return await RummyPoolCardDrop.findOne({
                attributes: ["card"],
                where: {
                    game_id: gameId
                },
    ...
    async totalAmountOnTable(tableId) {
        try {
            const result = await RummyPool.findOne({
    ...
    async updateTotalWinningAmtTable(amount, userWinningAmount, adminWinningAmount, tableId, winnerId) {
        try {
            return await RummyPoolTable.update(
                {
    ...
    async invested(tableId, seatPosition) {
        try {
            const sql = `
            SELECT SUM(tbl_rummy_pool_log.amount) AS amount,
            tbl_rummy_pool_table_user.seat_position,
            tbl_rummy_pool_table_user.user_id 
            FROM 
                tbl_rummy_pool_table_user 
            JOIN 
                tbl_rummy_pool 
                ON tbl_rummy_pool_table_user.table_id = tbl_rummy_pool.table_id 
            JOIN 
                tbl_rummy_pool_log 
                ON tbl_rummy_pool.id = tbl_rummy_pool_log.game_id 
                AND tbl_rummy_pool_log.user_id = tbl_rummy_pool_table_user.user_id 
            WHERE 
                tbl_rummy_pool_table_user.table_id = :tableId
                AND tbl_rummy_pool_table_user.seat_position = :seatPosition
                AND tbl_rummy_pool_log.amount > 0
        `;
            ...
            /*const result = await RummyPoolTableUser.findOne({
                attributes: [
                    [Sequelize.fn('SUM', Sequelize.col('RummyPoolLog.amount')), 'amount'],
                    'seat_position',
                    'user_id'
                ],
                include: [
                        ...
                        include: [
                            {
                                ...
                                where: {
                                    ...
                                    user_id: Sequelize.col('tbl_rummy_pool_table_user.user_id')
                                }
                            }
                        ]
                    }
                ...
                group: ['tbl_rummy_pool_table_user.seat_position', 'tbl_rummy_pool_table_user.user_id']
            });*/
...
module.exports = new RummyPoolService();