<?php $__env->startSection('title', 'Manual Deposit Requests'); ?>
<?php $__env->startSection('heading', 'Manual Deposit Requests'); ?>
<?php $__env->startSection('subheading', 'User UPI / Bank manual deposit requests — approve or reject each one'); ?>
...
<div class="panel">
    <div class="panel-body">
        ...
        <form method="GET" action="<?php echo e(route('admin.manual-deposits.index')); ?>" class="search-bar">
            <input type="text" name="search" value="<?php echo e($search ?? ''); ?>" placeholder="Search username, mobile, UTR...">
            <input type="hidden" name="tab" value="<?php echo e($tab); ?>">
            <button type="submit" class="s-btn">🔍 Search</button>
            <?php if($search): ?>
                <a href="<?php echo e(route('admin.manual-deposits.index')); ?>" class="c-btn">✕ Clear</a>
            <?php endif; ?>
        </form>

        
        <div class="tab-bar">
            <button class="tab-btn <?php echo e($tab === 'pending'  ? 'active' : ''); ?>" onclick="switchTab('pending',this)">
                Pending <span class="count-badge"><?php echo e($pending->count()); ?></span>
            </button>
            <button class="tab-btn <?php echo e($tab === 'approved' ? 'active' : ''); ?>" onclick="switchTab('approved',this)">
                Approved <span class="count-badge"><?php echo e($approved->count()); ?></span>
            </button>
            <button class="tab-btn <?php echo e($tab === 'rejected' ? 'active' : ''); ?>" onclick="switchTab('rejected',this)">
                Rejected <span class="count-badge"><?php echo e($rejected->count()); ?></span>
            </button>
        </div>

        <?php $__currentLoopData = ['pending' => $pending, 'approved' => $approved, 'rejected' => $rejected]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tabKey => $rows): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div id="tab-<?php echo e($tabKey); ?>" class="tab-section <?php echo e($tab === $tabKey ? 'active' : ''); ?>" style="border:1px solid #dee2e6; border-top:none; padding:16px;">
            <?php if($rows->isEmpty()): ?>
                <p class="empty-msg">📭 No <?php echo e($tabKey); ?> deposits found.</p>
            <?php else: ?>
            <div class="dep-wrap">
            <table class="dep-table">
                ...
                <?php $__currentLoopData = $rows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr>
                        ...
                        <td><?php echo e($row->user_name ?? 'ID:'.$row->user_id); ?></td>
                        <td><?php echo e($row->user_mobile ?? '-'); ?></td>
                        <td><?php echo e(number_format($row->price ?? 0, 2)); ?></td>
                        <td><?php echo e($row->coin ?? '-'); ?></td>
                        <td style="max-width:160px; overflow:hidden; text-overflow:ellipsis;" title="<?php echo e($row->utr); ?>">
                            <?php echo e($row->utr ?: ($row->transaction_id ?: '-')); ?>

                        </td>
                        <td>
                            <?php if($row->photo): ?>
                                <img src="<?php echo e($row->photo); ?>" class="ss-img" onclick="openSS('<?php echo e($row->photo); ?>')" title="Click to enlarge">
                            <?php else: ?>
                        ...
                        <td><?php echo e($row->added_date ?? '-'); ?></td>
                        <td>
                            <?php if((int)$row->status !== 1): ?>
                                <button class="action-btn btn-approve" onclick="changeStatus(<?php echo e($row->id); ?>, 1, this)">✓ Approve</button>
                            <?php endif; ?>
                            <?php if((int)$row->status !== 2): ?>
                                <button class="action-btn btn-reject" onclick="changeStatus(<?php echo e($row->id); ?>, 2, this)">✗ Reject</button>
                            <?php endif; ?>
                            <?php if((int)$row->status !== 0): ?>
                                <button class="action-btn btn-pending" onclick="changeStatus(<?php echo e($row->id); ?>, 0, this)">↺ Pending</button>
                            <?php endif; ?>
                        </td>
                    </tr>
...
function changeStatus(id, status, btn) {
    ...
    fetch('<?php echo e(route("admin.manual-deposits.status")); ?>', {
        method: 'POST',
        headers: {
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Live-Code\Live-Rox-Ludo\games\backend_laravel\resources\views/admin/manual-deposits/index.blade.php ENDPATH**/ ?>