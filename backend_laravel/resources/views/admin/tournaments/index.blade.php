        ...
        <div class="modal-card">
            ...
            <form method="POST" action="{{ $formAction }}" class="stack">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;">
                    ...
                    <div><label>Bot Start Policy</label><select name="bot_start_policy" style="width:100%;padding:10px;border:1px solid #d9e1e7;border-radius:8px;"><option value="disabled" @selected(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'disabled')>Disabled</option><option value="fill_missing" @selected(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'fill_missing')>Fill Missing Seats</option><option value="replace_offline" @selected(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'replace_offline')>Replace Offline Players</option><option value="hybrid" @selected(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'hybrid')>Hybrid</option></select></div>
                    <div><label>Min Real Players To Start</label><input type="number" name="min_real_players_to_start" value="{{ old('min_real_players_to_start', $t?->min_real_players_to_start ?? 1) }}" min="1" max="4" style="width:100%;padding:10px;border:1px solid #d9e1e7;border-radius:8px;"></div>
                    <div><label>Bot Fill Delay (sec)</label><input type="number" name="bot_fill_after_seconds" value="{{ old('bot_fill_after_seconds', $t?->bot_fill_after_seconds ?? 8) }}" min="0" max="300" style="width:100%;padding:10px;border:1px solid #d9e1e7;border-radius:8px;"></div>
