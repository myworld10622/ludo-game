...
@section('title', $mode === 'edit' ? 'Edit Agent Withdraw Gateway' : 'Add Agent Withdraw Gateway')
@section('heading', $mode === 'edit' ? 'Edit Agent Withdraw Gateway' : 'Add Agent Withdraw Gateway')
@section('subheading', 'Assign withdraw gateway number for an agent')

@section('content')
<div class="panel">
    <form method="POST" action="{{ $mode === 'edit' ? route('admin.gateways.agent-withdraw.update', $row->id ?? 0) : route('admin.gateways.agent-withdraw.store') }}">
        @csrf
        @if ($mode === 'edit')
            @method('PUT')
        @endif
        ...
        <div class="mobile-actions" style="margin-top: 16px;">
            ...
            <a class="btn btn-secondary" href="{{ route('admin.gateways.agent-withdraw.index') }}">Cancel</a>
        </div>
    </form>
</div>
@endsection
