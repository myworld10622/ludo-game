    ...
    <div class="modal-card">
        ...
        <form method="POST" action="<?php echo e($editing ? route('panel.tournaments.update', $editing) : route('panel.tournaments.store')); ?>">
            ...
            <div class="form-grid">
                ...
                <div><label>Registration Start</label><input type="datetime-local" name="registration_start_at" value="<?php echo e(old('registration_start_at', optional($editing?->registration_start_at)->format('Y-m-d\\TH:i'))); ?>"></div>
                <div><label>Registration End</label><input type="datetime-local" name="registration_end_at" value="<?php echo e(old('registration_end_at', optional($editing?->registration_end_at)->format('Y-m-d\\TH:i'))); ?>"></div>
                <div><label>Tournament Start</label><input type="datetime-local" name="tournament_start_at" value="<?php echo e(old('tournament_start_at', optional($editing?->tournament_start_at)->format('Y-m-d\\TH:i'))); ?>" required></div>
                ...
                <div class="form-grid">
                    <?php for($slot = 1; $slot <= 5; $slot++): ?>
                        <div class="panel" style="padding:12px;">
                            ...
                            <div><label>Start</label><input type="datetime-local" name="play_slot_start_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_start_'.$slot, ($editing && data_get($editing->play_slots, ($slot - 1).'.start_at')) ? \Illuminate\Support\Carbon::parse(data_get($editing->play_slots, ($slot - 1).'.start_at'))->format('Y-m-d\\TH:i') : '')); ?>"></div>
                            <div style="margin-top:10px;"><label>End</label><input type="datetime-local" name="play_slot_end_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_end_'.$slot, ($editing && data_get($editing->play_slots, ($slot - 1).'.end_at')) ? \Illuminate\Support\Carbon::parse(data_get($editing->play_slots, ($slot - 1).'.end_at'))->format('Y-m-d\\TH:i') : '')); ?>"></div>
                        </div>
                    <?php endfor; ?>
                </div>
            </div>
            ...
            <div class="card-head">
                <div>
                    ...
                    <div class="muted" style="font-size:13px;">
                        Created <?php echo e($tournament->created_at?->format('d M Y, h:i A') ?? '—'); ?>
...
<?php echo $__env->make('user.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Live-Code\games\backend_laravel\resources\views/user/tournaments/index.blade.php ENDPATH**/ ?>