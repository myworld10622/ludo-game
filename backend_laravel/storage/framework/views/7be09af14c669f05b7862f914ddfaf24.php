    ...
    <div class="panel">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;">
            <?php $__empty_1 = true; $__currentLoopData = $recentTournamentReports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tournament): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <a href="<?php echo e(route('admin.tournaments.report', $tournament)); ?>" style="display:block;border:1px solid var(--line-dim);border-radius:14px;padding:16px;background:var(--card2);transition:border-color .15s;">
                    <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
                        <div>
                            <div style="font-size:18px;font-weight:700;color:var(--text);"><?php echo e($tournament->name); ?></div>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px;">
                        <div><div class="stat-label">Players</div><div style="font-weight:700;color:var(--text);"><?php echo e($tournament->registrations_count); ?>/<?php echo e($tournament->max_players); ?></div></div>
                        <div><div class="stat-label">Prize Pool</div><div style="font-weight:700;color:var(--gold);">₹<?php echo e(number_format((float) $tournament->total_prize_pool, 0)); ?></div></div>
                        <div><div class="stat-label">Completed</div><div style="font-weight:700;color:var(--text);"><?php echo e($tournament->completed_matches_count); ?></div></div>
                        <div><div class="stat-label">Pending</div><div style="font-weight:700;color:var(--text);"><?php echo e($tournament->pending_matches_count); ?></div></div>
                    </div>
                    <div class="muted" style="margin-top:12px;font-size:12px;">
                        Start: <span data-utc-time="<?php echo e($tournament->tournament_start_at?->toIso8601String()); ?>"><?php echo e($tournament->tournament_start_at?->format('d M Y, h:i A') ?? '—'); ?></span>
                    </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            max="100000"
                            step="1"
                            value="<?php echo e((int) old('max_players', $t?->max_players ?? 16)); ?>"
                            placeholder="Enter player cap up to 100000"
                        >
                    </div>
                    <div><label>Players Per Match</label><select name="players_per_match"><option value="4" <?php if((int) old('players_per_match', $t?->players_per_match ?? 4) === 4): echo 'selected'; endif; ?>>4 Players</option><option value="2" <?php if((int) old('players_per_match', $t?->players_per_match) === 2): echo 'selected'; endif; ?>>2 Players</option></select></div>
                    <div><label>Platform Fee %</label><input type="number" step="0.1" name="platform_fee_pct" value="<?php echo e(old('platform_fee_pct', $t?->platform_fee_pct ?? 20)); ?>"></div>
                    <div><label>Bracket Mode</label><select name="bracket_mode"><option value="auto" <?php if(old('bracket_mode', $t?->bracket_mode ?? 'auto') === 'auto'): echo 'selected'; endif; ?>>Auto</option><option value="manual" <?php if(old('bracket_mode', $t?->bracket_mode) === 'manual'): echo 'selected'; endif; ?>>Manual</option></select></div>
                    <div><label>Allow Bots</label><select name="bot_allowed"><option value="0" <?php if(! old('bot_allowed', $t?->bot_allowed ?? true)): echo 'selected'; endif; ?>>No</option><option value="1" <?php if((bool) old('bot_allowed', $t?->bot_allowed ?? true)): echo 'selected'; endif; ?>>Yes</option></select></div>
                    <div><label>Max Bot %</label><input type="number" step="1" name="max_bot_pct" value="<?php echo e(old('max_bot_pct', $t?->max_bot_pct ?? 5)); ?>" min="0" max="100"></div>
                    <div><label>Bot Start Policy</label><select name="bot_start_policy"><option value="disabled" <?php if(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'disabled'): echo 'selected'; endif; ?>>Disabled</option><option value="fill_missing" <?php if(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'fill_missing'): echo 'selected'; endif; ?>>Fill Missing Seats</option><option value="replace_offline" <?php if(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'replace_offline'): echo 'selected'; endif; ?>>Replace Offline Players</option><option value="hybrid" <?php if(old('bot_start_policy', $t?->bot_start_policy ?? 'hybrid') === 'hybrid'): echo 'selected'; endif; ?>>Hybrid</option></select></div>
                    <div><label>Min Real Players To Start</label><input type="number" name="min_real_players_to_start" value="<?php echo e(old('min_real_players_to_start', $t?->min_real_players_to_start ?? 1)); ?>" min="1" max="4"></div>
                    <div><label>Bot Fill Delay (sec)</label><input type="number" name="bot_fill_after_seconds" value="<?php echo e(old('bot_fill_after_seconds', $t?->bot_fill_after_seconds ?? 8)); ?>" min="0" max="300"></div>
                    <div><label>Registration Opens</label><input type="datetime-local" name="registration_start_at" value="<?php echo e(old('registration_start_at')); ?>" data-utc="<?php echo e($t?->registration_start_at?->toIso8601String()); ?>"></div>
                    <div><label>Registration Closes</label><input type="datetime-local" name="registration_end_at" value="<?php echo e(old('registration_end_at')); ?>" data-utc="<?php echo e($t?->registration_end_at?->toIso8601String()); ?>"></div>
                    <div><label>Tournament Start</label><input type="datetime-local" name="tournament_start_at" required value="<?php echo e(old('tournament_start_at')); ?>" data-utc="<?php echo e($t?->tournament_start_at?->toIso8601String()); ?>"></div>
                    <div><label>Private Password</label><input name="invite_password" value="<?php echo e(old('invite_password', $t?->invite_password)); ?>"></div>
                </div>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px;margin-top:10px;">
                        <?php for($slot = 1; $slot <= 5; $slot++): ?>
                            ...
                            <div class="panel" style="padding:12px;">
                                ...
                                <label>Start</label>
                                <input type="time" name="play_slot_start_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_start_'.$slot, $slotStartTime)); ?>">
                                <label style="margin-top:8px;">End</label>
                                <input type="time" name="play_slot_end_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_end_'.$slot, $slotEndTime)); ?>">
                            </div>
                        <?php endfor; ?>
                    </div>
                </div>

                <div><label>Terms & Conditions</label><textarea name="terms_conditions" rows="2"><?php echo e(old('terms_conditions', $t?->terms_conditions)); ?></textarea></div>
                    ...
                    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:10px;">
                        <?php for($pos = 1; $pos <= 5; $pos++): ?>
                            <div>
                                ...
                                <input type="number" step="0.1" name="prize_pct_<?php echo e($pos); ?>" value="<?php echo e(old("prize_pct_{$pos}", $prizePct($pos))); ?>" min="0" max="100">
                            </div>
                        <?php endfor; ?>
                    </div>
                </div>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $userTournaments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tournament): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr>
                            ...
                            <td data-label="Tournament">
                                <strong><?php echo e($tournament->name); ?></strong>
                                <div class="muted" style="font-size:12px;"><?php echo e(ucfirst($tournament->type)); ?> · <?php echo e(ucwords(str_replace('_',' ',$tournament->format))); ?> · <span data-utc-time="<?php echo e($tournament->created_at?->toIso8601String()); ?>"><?php echo e($tournament->created_at?->format('d M Y, h:i A')); ?></span></div>
                            </td>
                            ...
                            <td data-label="Actions" style="white-space:nowrap;">
                                ...
                                <?php if(!$tournament->is_approved): ?>
                                    <form method="POST" action="<?php echo e(route('admin.tournaments.approve', $tournament)); ?>" style="display:inline;"><?php echo csrf_field(); ?><button type="submit" class="btn" style="background:linear-gradient(135deg,var(--green),#028A5E);color:#000;font-size:12px;padding:4px 10px;margin-right:4px;">Approve</button></form>
                                <?php endif; ?>
                                <?php if($isDraft): ?>
                                    <form method="POST" action="<?php echo e(route('admin.tournaments.force-live', $tournament)); ?>" style="display:inline;"><?php echo csrf_field(); ?><button type="submit" class="btn" style="background:linear-gradient(135deg,#FF9500,#d97706);color:#000;font-size:12px;padding:4px 10px;margin-right:4px;">Force Live</button></form>
                                <?php endif; ?>
