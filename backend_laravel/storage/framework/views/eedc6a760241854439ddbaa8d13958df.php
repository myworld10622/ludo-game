    ...
    <div class="modal-card">
        ...
        <form method="POST" action="<?php echo e($editing ? route('panel.tournaments.update', $editing) : route('panel.tournaments.store')); ?>">
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <input
                        ...
                        max="100000"
                        step="1"
                        value="<?php echo e((int) old('max_players', $editing?->max_players ?? 16)); ?>"
            ...
            <div class="stack-compact" style="margin-top:14px;">
                ...
                <div class="muted">Optional daily preferred play windows. Users can still play outside these times.</div>
                <div class="form-grid">
                    <?php for($slot = 1; $slot <= 5; $slot++): ?>
                        <?php
                            $slotStart = data_get($editing?->play_slots, ($slot - 1).'.start_at');
                            $slotEnd = data_get($editing?->play_slots, ($slot - 1).'.end_at');
                            $slotStartTime = data_get($editing?->play_slots, ($slot - 1).'.start_time') ?: ($slotStart ? \Illuminate\Support\Carbon::parse($slotStart)->format('H:i') : '');
                            $slotEndTime = data_get($editing?->play_slots, ($slot - 1).'.end_time') ?: ($slotEnd ? \Illuminate\Support\Carbon::parse($slotEnd)->format('H:i') : '');
                        ?>
                        <div class="panel" style="padding:12px;">
                            <div style="font-size:14px;font-weight:700;margin-bottom:10px;">Slot <?php echo e($slot); ?></div>
                            <div><label>Start</label><input type="time" name="play_slot_start_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_start_'.$slot, $slotStartTime)); ?>"></div>
                            <div style="margin-top:10px;"><label>End</label><input type="time" name="play_slot_end_<?php echo e($slot); ?>" value="<?php echo e(old('play_slot_end_'.$slot, $slotEndTime)); ?>"></div>
                        </div>
                    <?php endfor; ?>
                </div>
            </div>
